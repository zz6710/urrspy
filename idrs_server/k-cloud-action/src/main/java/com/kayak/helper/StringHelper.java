...
public class StringHelper {
	...
	/**
	 * 如果字符串为null、''、和全部为空格，返回true，其他返回false
	 * @param str 字符串
	 * @return 是否为空标识
	 */
	...
	/**
	 * 如果字符串为null、''、和全部为空格，返回false，其他返回true
	 * @param str 字符串
	 * @return 是否为空标识
	 */
	...
	public static String getStringByArray(List al) {
		if(al == null) return "";
		StringBuilder buffer = new StringBuilder(al.size() * 10);

		for(int i = 0; i < al.size(); i++){
			buffer.append(al.get(i)).append(",");
		}
		buffer.deleteCharAt(buffer.length() - 1);
		return buffer.toString();
	}
