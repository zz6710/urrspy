...
@Component
@Scope("prototype")
public class AutoBatchSendRegularNotice extends BaseTaskService<PubReq, PubResp> {
    ...
    */
    @StepNo(stepNo = 1)
    protected void autoBatchSendRegularNotice(PubReq request) throws Exception{
        log.info("---------- 任务: " + request.getTaskId() +" 定期报告公告自动批量发布 Start -----------");
        ...
        log.info("---------- 任务: " + request.getTaskId() +" 定期报告公告自动批量发布 End-----------");
    }
