...
@Component
@Scope("prototype")
public class PbcSjcjReportIndexValidationService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(PbcSjcjReportIndexValidationService.class);

    public static int DELAYED_DAYS = 2;
    public static String DELAYED_TYPE = "1";//顺延类型,0-自然日 1-工作日
    ...
    protected void step1Process(PubReq request) throws Exception {
        ...
        log.info("---------- " + task_id + "人行资管产品采集报送指标校验:任务 开始执行 -----------");
        String base_date = request.getTaskDate();
        String deal_date = base_date;
        Map<String, String> params = new HashMap<>();
        params.put("reportType", "07");//07-人行数据采集
        params.put("isBatch", "01");
        params.put("report_freq", "1");//1-一维报表
        ...
        if (reportDataValidateService.isFitTheDelayDay(base_date, DELAYED_DAYS, deal_date, DELAYED_TYPE)) {
            reportDataValidateService.execute(base_date, params);
        }

        log.info("---------- " + task_id + "人行资管产品采集报送指标校验:任务 执行结束 -----------");
    }
