 ...
 */
@Configuration
public class TaskUpdMonitorUtil {

    private final Logger log = LoggerFactory.getLogger(TaskUpdMonitorUtil.class);
    @Autowired
    private ApplicationContext ac;

    /* dps定时任务,资管数据在交易工作时间段每半小时同步一次 */
    @Scheduled(cron = "${pms.scheduled.do_task.updTime}")
    public void doRegistProcess() {
        ...
        try {
            //更新交易信息登记和资产负债要素登记
            ttrdInstitutionService.updateTradeInfoReportData();
        } catch (Exception e) {
