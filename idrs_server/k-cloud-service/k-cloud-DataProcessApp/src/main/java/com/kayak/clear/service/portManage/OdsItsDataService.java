...
@Component
@Scope("prototype")
public class OdsItsDataService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(OdsItsDataService.class);
    ...
    protected void process(PubReq request) throws Exception {
        log.info("---------- 投资交易系统贴源层数据处理 Start -----------");
        log.info(request.getTaskId() + " Request: {}", request);
        Map<String, Object> params =new HashMap<>();
        params.put("portType", STGConstants.STG_PORT_TYPE_JY);//接口类型：1-估值fa 2-投资交易 3-理财销售ta 4-万得wind 5-资管amng
        ...
        /** 查询当天是否为工作日 */
        SqlResult sqlResult = dbop.select("M080712", new ParamMap().on("currWorkdate", workDate));
        if(sqlResult.getRowSize() == 0 ){
            log.info("非工作日投资交易系统不推送文件");
        }else{
            dealPortFileService.dealAllPortInfo(params);
        }
        log.info("---------- 投资交易系统贴源层数据处理 End -----------");
    }
