...
public class FileZipUtils {
    ...
    public static void unZipFile(String zippath, String targetpath) throws Exception {
        ...
        try (ZipFile _zipFile = new ZipFile(zipFile, Charset.forName("GBK"))) {
            for (Enumeration<? extends ZipEntry> entries = _zipFile.entries(); entries.hasMoreElements();) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try (InputStream _in = _zipFile.getInputStream(entry);
                         OutputStream _out = new FileOutputStream(targetFile)) {
                        int len;
                        while ((len = _in.read(buffer)) > 0) {
                            _out.write(buffer, 0, len);
                        }
                    } catch (IOException e) {
                        throw new IOException("解压条目 [" + entry.getName() + "] 失败", e);
                    }
                }
            }
        } catch (IOException e) {
