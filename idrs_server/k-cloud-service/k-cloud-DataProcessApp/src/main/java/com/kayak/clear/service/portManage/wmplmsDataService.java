...
@Component
@Scope("prototype")
public class wmplmsDataService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(wmplmsDataService.class);
    ...
    protected void process(PubReq request) throws Exception {
        if(TaskUtil.isSkipTask(request.getTaskDate(),request.getTaskId())) return; //判断非工作日不执行

        log.info("---------- WMPLMS接口数据处理 Start -----------");
        ...
        List<SqlRow> list01 = comnDao.findRows("select port_code, port_name, port_exeid, port_address, port_state, " +
                "file_type, port_type, port_dir, port_table  from base_port_manage where pid = $S{pId}  ", params);
        if(list01 != null && list01.size() > 0 ){
            for(SqlRow sqlRow : list01){
                params.put("portType", sqlRow.getString("port_type"));
                params.put("portDir", sqlRow.getString("port_dir"));
                try {
            ...
            params.put("portType", STGConstants.STG_PORT_TYPE_PD);
