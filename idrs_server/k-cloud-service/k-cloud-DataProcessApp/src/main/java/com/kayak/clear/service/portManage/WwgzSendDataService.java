...
@Component
@Scope("prototype")
public class WwgzSendDataService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(WwgzSendDataService.class);
    ...
    protected void process(PubReq request) throws Exception {
        log.info("---------- 向委外估值系统发送数据文件 Start -----------");
        log.info(request.getTaskId() + " Request: {}", request);
        Map<String, Object> params =new HashMap<>();
        params.put("portType", STGConstants.STG_PORT_TYPE_WWGZ);
        params.put("portDir", STGConstants.STG_DATA_DIR_SEND); //发送文件 程晓鹏 2025.01.14 modify
