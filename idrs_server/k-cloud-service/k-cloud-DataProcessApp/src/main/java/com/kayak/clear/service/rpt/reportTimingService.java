...
import com.kayak.dps.app.service.DealReportTimingService;
...
@Component
@Scope("prototype")
public class reportTimingService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(reportTimingService.class);

    @Autowired
    public CreateTaskService createTaskService;

    @Autowired
    private DealReportTimingService dealReportTimingService;
    ...
    protected void process(PubReq request) throws Exception {
        log.info("---------- 理财销售系统报表报送时点理论报送结束日生成数据处理 Start -----------");
        log.info(request.getTaskId() + " Request: {}", request);
        Map<String, Object> params =new HashMap<>();
        params.put("portType", STGConstants.STG_PORT_TYPE_TA);//接口类型：1-估值fa 2-投资交易 3-理财销售ta 4-万得wind 5-资管amng
        ...
        dealReportTimingService.dealAllPortInfo(params);

        log.info("----------  理财销售系统报表报送时点理论报送结束日生成数据处理 End -----------");
    }
