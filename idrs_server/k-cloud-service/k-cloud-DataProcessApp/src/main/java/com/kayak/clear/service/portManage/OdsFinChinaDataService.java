...
@Component
@Scope("prototype")
public class OdsFinChinaDataService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(OdsFinChinaDataService.class);
    ...
    protected void process(PubReq request) throws Exception {
        log.info("---------- 财汇数据平台贴源层数据处理 Start -----------");
        log.info(request.getTaskId() + " Request: {}", request);
        Map<String, Object> params =new HashMap<>();
        params.put("portType", STGConstants.STG_PORT_TYPE_CH);//接口类型：1-估值fa 2-投资交易 3-理财销售ta 4-万得wind 5-资管amng 6-内部文件生成 8-财汇数据平台
