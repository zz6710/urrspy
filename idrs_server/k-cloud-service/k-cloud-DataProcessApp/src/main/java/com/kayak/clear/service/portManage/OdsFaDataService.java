...
@Component
@Scope("prototype")
public class OdsFaDataService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(OdsFaDataService.class);
    ...
    protected void process(PubReq request) throws Exception {
        log.info("---------- 估值系统贴源层数据处理 Start -----------");
        log.info(request.getTaskId() + " Request: {}", request);
        Map<String, Object> params =new HashMap<>();
        params.put("portType", STGConstants.STG_PORT_TYPE_GZ);//接口类型：1-估值fa 2-投资交易 3-理财销售ta 4-万得wind 5-资管amng
