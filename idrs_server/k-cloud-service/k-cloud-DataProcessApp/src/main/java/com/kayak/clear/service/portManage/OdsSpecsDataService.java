...
@Component
@Scope("prototype")
public class OdsSpecsDataService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(OdsSpecsDataService.class);
    ...
    protected void process(PubReq request) throws Exception {
        log.info("---------- 规格系统贴源层数据处理 Start -----------");
        log.info(request.getTaskId() + " Request: {}", request);
        Map<String, Object> params =new HashMap<>();
        params.put("portType", "9");//接口类型：1-估值fa 2-投资交易 3-理财销售ta 4-万得wind 5-资管amng 9-规格
        params.put("portDir", "2");//接口方向：1-发送 2-接收
        params.put("dealDate", request.getTaskDate());//处理日期
        params.put("dealType", "1");//按天处理
