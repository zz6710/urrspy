 ...
 * 按月频率计算归档时间
 */
@Service("BakTypeMonthExt")
public class BakTypeMonthExt extends BakTypeAbstract{
    ...
    public BakConfigDTO getNextDate(BakConfigDTO dto, BakConfigDTO newDto) throws ParseException {
        ...
        if (ObjectUtil.isEmpty(dto.getNextDate())) {
            newDto.setNextDate(
                    BakDateUtil.getNextMonthFirstDay(dto.getBeginDate(), dto.getFrequency())
            );
        } else {
            //上次归档日期非空，通上次归档日期计算下次归档日期
            newDto.setLastDate(newDto.getNextDate());
            newDto.setNextDate(
                    BakDateUtil.getNextMonthFirstDay(dto.getNextDate(), dto.getFrequency())
            );
        }
        return newDto;
    }
