...
@Component
@Scope("prototype")
public class ZZSQReportIndexValidationService extends BaseTaskService<PubReq, PubResp> {

    private static Logger log = LoggerFactory.getLogger(ZZSQReportIndexValidationService.class);
    ...
    protected void step1Process(PubReq request) throws Exception {
        ...
        log.info("---------- " + task_id + "中债三期报送指标校验:任务 开始执行 -----------");
        String base_date = request.getTaskDate();
        Map<String, String> params = new HashMap<>();
        params.put("reportType", "01");
        params.put("isBatch", "01");
        params.put("report_freq", "");
        if("V012".equals(task_id)){
            params.put("reportTable", "app_cust_register_info");
        }
        if("V013".equals(task_id)){
            params.put("reportTable", "app_cust_trans_info");
        }
        if("V014".equals(task_id)){
            params.put("reportTable", "app_cust_vol_register_info");
        }
        reportDataValidateService.execute(base_date, params);

        log.info("---------- " + task_id + "中债三期报送指标校验:任务 执行结束 -----------");
    }
