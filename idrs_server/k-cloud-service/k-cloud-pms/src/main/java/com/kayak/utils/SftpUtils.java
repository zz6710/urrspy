...
public class SftpUtils {
    ...
    public static void deleteSFTP(String ip, String username, String password, String remotePath, String directory) throws Exception {
        log.info(" >>>>> SFTP开始删除文件夹");
        log.info(" >>>>> 远程地址：{}", ip);
        log.info(" >>>>> 远程目录：{}", remotePath);
        log.info(" >>>>> 文件夹名：{}", directory);
        ChannelSftp sftp = null;
        try {
            sftp = SftpUtils.connect(ip, 22, username, password, remotePath);
            if (!SftpUtils.isDirExist(directory, sftp)) {
                throw new RuntimeException("服务器文件夹不存在: " + remotePath + directory);
            }
