...
@Repository
public class ZG12Dao extends ComnDao {

    //TODO 查询条件需要控制，数据量大时可能会出现问题
    public SqlResult<ZG12> findZG12s(SqlParam<ZG12> params) throws Exception {
        String sql = "select  t.id,t.prod_cd,t.report_date,t.borrower_typ,t.zone,t.borrower_cd,t.industry_msg,t.enter_sponsor_eco_sector,t.enter_scale,t.borrower_asset_cd,t.borrower_real_trans,t.borrower_issue_dt,t.borrower_end_dt,t.borrower_extension_dt,t.is_fixed_rate,t.rate_level,t.guarantee_mode,t.ori_contract_ccy,t.ori_contract_amt,t.ori_contract_amt_cny,t.borrower_balance_ccy,t.borrower_balance,t.borrower_balance_cny,t.type_of_debt,t.register_trading_place,t.register_trading_code,t.register_status,t.theory_report_start_date,t.theory_report_end_date,t1.zone_text,t.tech_flag,t.green_flag,t.spec_flag,t.aged_flag,t.num_core_flag  from  app_pbc_report_ZG12 t left join (select '000000' as itemkey,'000000:全部地区' as zone_text from dual union all select itemkey,CONCAT(itemkey,':',itemval) as zone_text from sys_dict_item where dict  in ('pbc_city_area_det','pbc_country_code')) t1 on t.zone = t1.itemkey  where sys_data_status ='1' ";
    ...
    public UpdateResult updateZG12(SqlParam<ZG12> params) throws Exception {
        return super.update("update  app_pbc_report_ZG12 " +
                        "set prod_cd=$S{prodCd}," +
                        "borrower_typ=$S{borrowerTyp}," +
                        "zone=$S{zoneText}," +
                        "borrower_cd=$S{borrowerCd}," +
                        "industry_msg=$S{industryMsg}," +
                        "enter_sponsor_eco_sector=$S{enterSponsorEcoSector}," +
                        "enter_scale=$S{enterScale}," +
                        "borrower_asset_cd = $S{borrowerAssetCd}," +
                        "borrower_real_trans = $S{borrowerRealTrans}," +
                        "borrower_issue_dt = $S{borrowerIssueDt}," +
                        "borrower_end_dt = $S{borrowerEndDt}," +
                        "borrower_extension_dt = $S{borrowerExtensionDt}," +
                        "is_fixed_rate=$S{isFixedRate}," +
                        "rate_level=$S{rateLevel}," +
                        "guarantee_mode=$S{guaranteeMode}," +
                        "ori_contract_ccy=$S{oriContractCcy}," +
                        "ori_contract_amt=$S{oriContractAmt}," +
                        "ori_contract_amt_cny=$S{oriContractAmtCny}," +
                        "borrower_balance_ccy = $S{borrowerBalanceCcy}," +
                        "borrower_balance = $S{borrowerBalance}," +
                        "borrower_balance_cny = $S{borrowerBalanceCny}," +
                        "type_of_debt = $S{typeOfDebt}," +
                        "register_trading_place = $S{registerTradingPlace}," +
                        "register_trading_code = $S{registerTradingCode}," +
                        "tech_flag=$S{techFlag},green_flag=$S{greenFlag},spec_flag=$S{specFlag},aged_flag=$S{agedFlag},num_core_flag=$S{numCoreFlag}," +
                        "theory_report_start_date=$S{theoryReportStartDate}  where id=$S{id}",
                params.getModel());
    }

    public UpdateResult deleteZG12(SqlParam<ZG12> params) throws Exception {
        return super.update("DELETE FROM app_pbc_report_ZG12 WHERE id = $S{id}",
                params.getModel());
    }

    public void deleteZg12ByDate(Object params) throws Exception {

        super.update("DELETE FROM app_pbc_report_ZG12 where report_date = $S{beginDate} ", params);
    }

    public UpdateResult addZG12(Object params) throws Exception {
        return super.update("INSERT INTO app_pbc_report_ZG12(prod_cd,loan_typ,loan_trans_org_cd,loan_contract_ori_cd,loan_contract_ori_zone,borrower_typ,zone,borrower_cd,industry_msg,enter_sponsor_eco_sector,enter_scale,loan_receipt_cd,loan_prod_typ,loan_real_trans,loan_issue_dt,loan_end_dt,loan_extension_dt,is_fixed_rate,rate_level,guarantee_mode,loan_quality,loan_status,loan_trans_discount_rate,ori_contract_ccy,ori_contract_amt,ori_contract_amt_cny,loan_balance_ccy,loan_balance,loan_balance_cny,register_status,theory_report_start_date) VALUES($S{prodCd},$S{loanTyp},$S{loanTransOrgCd},$S{loanContractOriCd},$S{loanContractOriZone},$S{borrowerTyp},$S{zone},$S{borrowerCd},$S{industryMsg},$S{enterSponsorEcoSector},$S{enterScale},$S{loanReceiptCd},$S{loanProdTyp},$S{loanRealTrans},$S{loanIssueDt},$S{loanEndDt},$S{loanExtensionDt},$S{isFixedRate},if($S{rateLevel}='',null,$S{rateLevel}),$S{guaranteeMode},$S{loanQuality},$S{loanStatus},if($S{loanTransDiscountRate}='',null,$S{loanTransDiscountRate}),$S{oriContractCcy},if($S{oriContractAmt}='',null,$S{oriContractAmt}),if($S{oriContractAmtCny}='',null,$S{oriContractAmtCny}),$S{loanbalanceccy},if($S{loanBalance}='',null,$S{loanBalance}),if($S{loanBalanceCny}='',null,$S{loanBalanceCny}),$S{registerStatus},$S{theoryReportStartDate})", params);
    }
