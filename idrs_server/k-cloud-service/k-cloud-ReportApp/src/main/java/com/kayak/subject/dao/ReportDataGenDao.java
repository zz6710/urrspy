...
import com.kayak.subject.exception.ReportDataProcessExecuteException;
...
public class ReportDataGenDao extends ComnDao {
    ...
    public String getReportAppDataVersionByDate (String report_date, String report_table, String data_type) throws Exception{
        ...
        if (!"".equals(sqlRow.getString("cur_version")) && "1".equals(data_type)) {//数据类型为计算重新往上堆叠版本
        ...
        } else if (!"".equals(sqlRow.getString("cur_version")) && "2".equals(data_type)) {//汇总是对上次计算结果进行汇总,版本取上一个计算版本,既最大版本号
