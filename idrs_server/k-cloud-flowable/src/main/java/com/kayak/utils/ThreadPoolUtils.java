...
import com.kayak.config.properties.ThreadPoolProperties;
    ...
    public static ThreadPoolExecutor getThreadPool() {
        if (pool == null) {
            synchronized (ThreadPoolUtils.class) {
                if (pool == null) {
                    log.info("开辟程池！！！！！");
                    ThreadPoolProperties properties = threadPoolUtils.threadPoolProperties;
                    pool = new ThreadPoolExecutor(
                            properties.getMinCount(),
                            properties.getMaxCount(),
                            0L,
                            TimeUnit.MILLISECONDS,
                            new LinkedBlockingQueue<>(properties.getBlockingQueueCount()));
                }
            }
        }
        return pool;
