...
public class FlowableUtils {
     ...
     * @return
     */
    public static FlowElement findFlowElementByActivityId(String processDefId, String activityId, RepositoryService repositoryService) {
        FlowElement retFlowElement = null;
        BpmnModel bpmnModel = repositoryService.getBpmnModel(processDefId);
        List<Process> processes = bpmnModel.getProcesses();
        for (Process process : processes) {
            FlowElement flowElement = process.getFlowElementMap().get(activityId);
            if (flowElement != null) {
                retFlowElement = flowElement;
            }
        }
        return retFlowElement;
    }
     ...
     * @return
     */
    public static FlowElement findFlowElementByActivityId(BpmnModel bpmnModel, String activityId) {
        FlowElement retFlowElement = null;
        List<Process> processes = bpmnModel.getProcesses();
        for (Process process : processes) {
            FlowElement flowElement = process.getFlowElementMap().get(activityId);
