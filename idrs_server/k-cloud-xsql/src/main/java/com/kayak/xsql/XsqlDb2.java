 ...
 * 
 */
public class XsqlDb2 extends XsqlImpl {
	private static final Logger log = LoggerFactory.getLogger(XsqlDb2.class);
	...
	public <T> List<T> page(Class<T> clazz, int offset, int limit, String sql, Object... params) throws Exception {
		...
		try {
			...
			String paged = "select * from (select xsql_t1.*, rownumber() over() as xsql_rownum from ("
					+ ph.getSql(param) + ") xsql_t1 ) xsql_t2 where xsql_rownum between " + offset + " and "
					+ (offset + limit);
