...
public class DateUtil {
	...
	public static String DATE_CAL_FREQ_SEASON = "3";//季度系数
	public static String DATE_CAL_FREQ_MONTH = "1";//月度系数
	...
	public static String getCurrentDate(){
		SimpleDateFormat predf = new SimpleDateFormat("yyyyMMdd");
		Date d=new Date();
		return predf.format(new Date(d.getTime()));
	}
	...
	public static String getFirstDayDateOfMonth(String date) throws ParseException {
		...
		cal.setTime(DateUtil.getDate(date, sd));
    ...
    * */
	public static String getCurrentQuarterNumbers(){
        ...
        String quarterNumber = DateUtil.getQuarterNumber(date,0);
        String baseDate = "";
        String year = DateUtil.getNowDate1().substring(0,4);
        //当前日期 执行的是 上个季度公告
        if ("1".equals(quarterNumber)) {
            year = DateUtil.getDayByYear();
            baseDate= year +"1231";
	...
	/**
	 * @Description 获取nowTime日期month个月前的的数据
	 * @Param []
	...
	/**
	 * @Description 获取指定日期前一天的数据
	 * @Param []
	 * @Return java.lang.String`
	 */
	public static String geBeforeDay(String day) throws ParseException{
		SimpleDateFormat predf = new SimpleDateFormat("yyyyMMdd");
	 ...
	 * 日期格式为 yyyy-MM
	 */
	public static String getMonthEndDate(String mth_date) throws ParseException {
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
		SimpleDateFormat dateFormat1 = new SimpleDateFormat("yyyy-MM-dd");
		Calendar cal = Calendar.getInstance();
		cal.setTime(dateFormat1.parse(mth_date + "-01"));
		cal.set(Calendar.DAY_OF_MONTH,cal.getActualMaximum(Calendar.DAY_OF_MONTH));
		return dateFormat.format(cal.getTime());
	}

	//是否为工作日
	public static boolean isWordDay (String day) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str = "select * from sys_workday_set where pgmno = '001' and workday = $S{day} order by workday asc limit 1";
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day);
	 ...
	 * @throws Exception
	 */
	public static String getNextSysWordDay(String day) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str = "select * from sys_workday_set where pgmno = '001' and workday > $S{day} order by workday asc limit 1";
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day);
			if (sqlRows != null && sqlRows.size() > 0)
				return sqlRows.get(0).getString("workday");
			else
				return DateUtil.getAfterDay(day);
		}
	}
	/**
	 * axin
	 ...
	 * @throws Exception
	 */
	public static String getLastSysWordDay(String day) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str = "select * from sys_workday_set where pgmno = '001' and workday < $S{day} order by workday desc limit 1";
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day);
			if (sqlRows != null && sqlRows.size() > 0)
				return sqlRows.get(0).getString("workday");
			else
				return DateUtil.geBeforeDay(day);
		}
	}
	 ...
	 * @throws Exception
	 */
	public static String addSysWordDay(String day,int amount) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			if(amount==0){
				return day;
			}
			String str = "select * from sys_workday_set where pgmno = '001' and workday < $S{day} order by workday desc ";
			if(amount>=0){
				   str = "select * from sys_workday_set where pgmno = '001' and workday > $S{day} order by workday";
			}else{
				amount = Math.abs(amount);
			}
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day);
			if (sqlRows != null && sqlRows.size() > 0){
				return sqlRows.get(amount-1).getString("workday");
			}else{
				return day;
			}
		}
	...
	public static String dateFormat(String date,String pattern) throws Exception {
		...
		String d = new SimpleDateFormat(pattern).format(dateFormat);
		return d;
	}

	public static int getWeek(String date) throws ParseException {

		Date bdate = SDF_DATE_8.parse(date);
		Calendar cal = Calendar.getInstance();
		cal.setTime(bdate);
	 ...
	 *  根据传入时间 获取上周时间
	 * @param time 日期
	 * @param week Calendar.FRIDAY 周一
	 * @return
	 */
	public static String getLastWeekMonday(String time , int week) throws ParseException {

		Date date = SDF_DATE_8.parse(time);
		...
		return SDF_DATE_8.format(cal.getTime()) ;
	}
	...
	public static List<String> getDateListDay(String date1, String date2,String cycleOpenTerm) throws ParseException {
		...
		Date d1 = SDF_DATE_8.parse(date1);
		Date d2 = SDF_DATE_8.parse(date2);
		Calendar cr = Calendar.getInstance();
		while ((d1.getTime() - d2.getTime()) < 0) {
			cr.clear();
			cr.setTime(d1);
			dates.add(SDF_DATE_8.format(d1));
...
//        }
	...
	public static List<String> getDateListForWeek(String date1, String date2,String cycleOpenTerm,
		...
		LocalDate yyyyMMdd = LocalDate.parse(date1, DateTimeFormatter.ofPattern("yyyyMMdd"));
		DayOfWeek dayOfWeek = yyyyMMdd.getDayOfWeek();
		if (dayOfWeek.equals(DayOfWeek.of(Integer.parseInt(orderOpenDays)))){
			date1 = yyyyMMdd.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
		}else {
			...
			date1 = localDate1.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
		}
...
//        }
	 ...
	 * @param date1,起始日期
	 * @param date2 结束日期
	 * @param cycleOpenTerm 规则1，每 cycleOpenTerm 月
	 * @param orderOpenDays 规则2，几号
	 * @return
	 * @throws ParseException
	 */
	public static List<String> getDateListForMonth(String date1, String date2,String cycleOpenTerm,
		...
		if (localDate.getDayOfMonth() >= yyyyMMdd.getDayOfMonth()){
			date1 = localDate.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
		}else{
			LocalDate localDate2 = yyyyMMdd.with(MonthDay.of(yyyyMMdd.minusMonths(-1).getMonth(),Integer.parseInt(orderOpenDays)));
			date1 = localDate2.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
		}
...
//        }
	...
	public static String getMaxDate(String beginDateStr, String endDateStr) throws Exception {
		Date beginDate = parse(beginDateStr,"yyyyMMdd");
		Date endDate = parse(endDateStr,"yyyyMMdd");
		if(beginDate==null) {
			return toDate(endDate,"yyyyMMdd");
		}
		if(endDate==null) {
			return toDate(beginDate,"yyyyMMdd");
		}
	 ...
	 * @throws ParseException
	 */
	public static boolean isFirstWordDay (String day) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str = "select * from sys_workday_set where pgmno = '001' and date_format(workday, '%Y%m') = date_format($S{day}, '%Y%m') order by workday asc limit 1";
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day);
			if (sqlRows.size() > 0){
				for (SqlRow sqlRow :sqlRows) {
					String thisWorkday = sqlRow.getString("workday");
					if(day.equals(thisWorkday)){
						return true;
					}
				}
			}
	 ...
	 * @return
	 */
	public static String geMonthEndDay(String day) throws ParseException {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
	 ...
	 * @return
	 */
	public static String getNextWordDay(String day) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str = "select * from sys_workday_set where pgmno = '001' and workday >= $S{day} order by workday asc limit 1";
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day);
	...
	/**
	 * 根据基准日期类型和任务日期计算出报送数据日期
	 * @param settle_date 任务日期
	 * @param base_type 基准日期类型 06-月度末 07-季度末
	 * @param gen_days 数据生成日期(工作日)
	 * @return
	 * @throws Exception
	 */
	public static String calReportDateByDateType(String settle_date, String base_type, int gen_days) throws Exception {
		int coefficient = 1;//默认按月
		coefficient = base_type.equals(DateUtil.DATE_CAL_FREQ_SEASON)?3:1;
		return (gen_days==0)?getLastCycleDay(settle_date,0, coefficient):getLastCycleDay(settle_date,-1, coefficient);
	}

	/**
	 * 计算指定日期T-N月末日期
	 * @param settle_date 指定日期
	 * @param month_add 减去月份数
	 * @param coefficient 系数 1-按月 3-按季 6-按半年 12-按年
	 * @return
	 * @throws Exception
	 */
	public static String getLastCycleDay (String settle_date, int month_add, int coefficient) throws Exception {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		Date date = sdf.parse(settle_date);
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
		calendar.add(Calendar.MONTH, month_add * coefficient);
		calendar.set(Calendar.DAY_OF_MONTH, calendar.getActualMaximum(Calendar.DAY_OF_MONTH));
		return sdf.format(calendar.getTime());
	}

	/**
	 * 功能：获取指定日期(年月)所在月份的最后一天
	 *
	 * @param settle_date
	 * @return
	 * @throws ParseException
	 */
	public static String getLastDayOfYearMonth(String settle_date) throws ParseException {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		Calendar calendar = Calendar.getInstance();
		Date date = sdf.parse(settle_date+"01");
		calendar.setTime(date);
		calendar.set(Calendar.DAY_OF_MONTH,calendar.getActualMaximum(Calendar.DAY_OF_MONTH));
	 ...
	 * @throws Exception
	 */
	public static String getSysWordDayByNum(String day, int num) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str ="";
			if(num>0){
				num = Math.abs(num);
				str = " select * from (select row_number() over (partition by pgmno order by workday asc  ) as rn,workday  from sys_workday_set where pgmno = '001' and workday > $S{day} ) a where a.rn =$S{num}   ";
			}else{
				num = Math.abs(num);
				str = " select * from (select row_number() over (partition by pgmno order by workday desc  ) as rn,workday  from sys_workday_set where pgmno = '001' and workday < $S{day} ) a where a.rn =$S{num}   ";
			}
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, day,num);
			if (sqlRows != null && sqlRows.size() > 0)
				return sqlRows.get(0).getString("workday");
			else
				return DateUtil.getAfterDay(day);
	...
	public static String getLastWorkday(String currentDay) throws Exception {
		DaoService daoService = SysBeans.getBean("daoService");
		try (AutoCloseable ca = daoService.selectDataSource(0)) {
			String str = "SELECT max(workday) workday FROM sys_workday_set WHERE pgmno = '001' and workday <=$S{currentDay} order by workday ";
			List<SqlRow> sqlRows = daoService.list(SqlRow.class, str, currentDay);
			if (sqlRows != null && sqlRows.size() > 0)
				return sqlRows.get(0).getString("workday");
			else
				return currentDay;
	 ...
	 * @return
	 */
	public static String getPreviousMonthLastDay(String date, int index){
		SimpleDateFormat df=new SimpleDateFormat("yyyyMMdd");
		Calendar cal=Calendar.getInstance();
		Date d;
		try {
	 ...
	 * @throws Exception
	 */
	public static List<String> calRangeDaysFromNatureDay (String start_date, String end_date,String range_type) throws Exception{
		//对日期进行排序，避免筛选范围限制
		String b_date = String.valueOf(Math.max(Integer.parseInt(start_date), Integer.parseInt(end_date)));
		String s_date = String.valueOf(Math.min(Integer.parseInt(start_date), Integer.parseInt(end_date)));

		List<String> taskDateList = new ArrayList<>();
		taskDateList.add(start_date);

		if ("02".equals(range_type)) {//02-自然日
			return calNatureRangeDateListByBorder(s_date,b_date,"02");
		} else if ("01".equals(range_type)) {//02-工作日
			//Todo 暂无业务场景不实现，自然日区间选取工作日
		}
	 ...
	 * @throws ParseException
	 */
	public static List<String> calNatureRangeDateListByBorder(String startDate, String endDate, String date_type) throws ParseException {
		int times = 0;
		String cur_date = "";
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		Calendar cal = Calendar.getInstance();
		Date sub_date = sdf.parse(startDate);

		List<String> taskDateList = new ArrayList<>();
		taskDateList.add(startDate);

		if ("02".equals(date_type)) {//02-取区间自然日
			cal.setTime(sub_date);
			while (startDate.compareTo(endDate) < 0 && times < 10000) {
				cal.add(Calendar.DATE, 1);
				startDate = cur_date = sdf.format(cal.getTime());
				taskDateList.add(cur_date);
				times++;
			}
		}
		return taskDateList;
	 ...
	 * @return
	 */
	public static String getNextMonthLastDay(String date, int index) throws Exception {
		SimpleDateFormat df=new SimpleDateFormat("yyyyMMdd");
		Calendar cal=Calendar.getInstance();
		Date d;
		try {
	 ...
	 * @throws Exception
	 */
	public static String getInvTable(String tableName, String reportDate) throws Exception {
		String table = "";
		if (getSysWordDay().equals(reportDate)) {
			// 当前工作日取当日表
			table = tableName;
		} else {
			// 非当前工作日取历史表
			table = tableName + "_" + reportDate;
		}
		return table;
	}
	 ...
	 * @throws Exception
	 */
	public static String getInvSubTable(String tableName, String reportDate) throws Exception {
		String table = "";
		if (getSysWordDay().equals(reportDate)) {
			// 当前工作日取当日表
			table = tableName;
