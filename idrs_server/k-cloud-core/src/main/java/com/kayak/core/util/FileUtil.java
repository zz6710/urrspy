...
public class FileUtil {
	...
	public static String getFilePath(String filename, ServletContext sc) {
		if (filename.startsWith("classpath:")) {
			...
			try {
				return com.kayak.core.util.FileUtil.class.getResource(filename).getFile();
			} catch (Exception e) {
	...
	public static void conmpressFiles(File zipFile,File Backup) throws Exception{
			...
			if (zipFile.exists()) {
				...
				if(systems.toLowerCase().startsWith("win")){
					...
					try {
						...
						log.error("set archive file:{} permision catch an error: {}", zipFile, e);
					}
				}
			}
		} finally {
	 ...
	 * @throws IOException
	 */
	public static void compressDir(ZipOutputStream zipOut, String baseDir) throws IOException {
	 ...
	 * 注意：如果只是操作这个文件，并非要保存这个文件，操作完成后需要调用 FileUtil.deleteFile(File file)
	 *
