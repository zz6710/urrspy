<template>
  <div class="grid-content">
    <md-card v-if="dataCard || dataCard === 'true'" class="k-card" :class="{'stickyStyle': dataSticky}">
        ...
        <el-popover style="float: right;" placement="bottom" width="180" trigger="click" v-if="dataEntity">
          <ul class="dropdown-column">
            <li v-for="(column,index) in girdColumns" :key="index">
              <span>
                <md-checkbox v-model="columnData" :value="column.dataName">{{ column.dataHeader }}</md-checkbox>
              </span>
            </li>
          </ul>
      ...
      <md-card-content>
        <div class="k-grid">
          <el-table v-if="!headerHide" ref="kgrid" :data="list" :row-key="tableOptions.dataTreeId"
                    :height="tableOptions.dataHeight"
                    :stripe="tableOptions.dataStripe" :expand-row-keys="dataExpendRow" :max-height="dataMaxHeight"
                    :border="true"
                    :default-expand-all="tableOptions.dataExpandAll === true || tableOptions.dataExpandAll === 'true'"
                    :lazy="dataLazy" :load="dataLoad"
                    @row-click="handleRowClick" @row-dblclick="handleDbRowClick"
                    @selection-change="handleSelectionChange"
                    style="width: 100%" @sort-change="sortChange" :default-sort="defaultSort"
            ...
            <el-table-column :fixed="dataFixed" v-if="this.$slots['expand'] || this.$scopedSlots['expand']"
                             type="expand">
            ...
            <el-table-column
              ...
              type="selection" :width="tableOptions.dataCheckboxWidth" :fixed="dataFixed" align="right"
              :reserve-selection="dataReserveSelection">
            </el-table-column>


            <el-table-column :width="operateWidth" :fixed="dataFixed" label="操作" v-if="(dataOperateColumn === true || dataOperateColumn === 'true')
            ...
            <template v-for="(column,index) in girdColumns">
              <k-grid-column v-if="column.dataShow" :key="index" :data-header="column.dataHeader"
                             :data-name="column.dataName" v-bind="column.dataExt"></k-grid-column>
            </template>
          ...
          <k-pagination v-if="dataPageSize > 0 && total>0 && !this.dataShowTree" :total="total" :page.sync="start"
                        :limit.sync="limit" @pagination="handlePagination" :layout="tableOptions.dataPaginationLayout"/>
        </div>
      </md-card-content>
    </md-card>
    <div v-else class="k-grid">
      ...
      <el-table v-if="!headerHide" ref="kgrid" :data="list" :row-key="tableOptions.dataTreeId"
                ...
                :default-expand-all="tableOptions.dataExpandAll === true || tableOptions.dataExpandAll === 'true'"
                :lazy="dataLazy" :load="dataLoad"
                @row-click="handleRowClick" @row-dblclick="handleDbRowClick" @selection-change="handleSelectionChange"
                style="width: 100%" @sort-change="sortChange" :default-sort="defaultSort"
                :row-class-name="tableRowClassName"
                @header-dragend="setWidth">
        ...
        <template v-for="(column,index) in girdColumns">
          <k-grid-column v-if="column.dataShow" :key="index" :data-header="column.dataHeader"
                         :data-name="column.dataName"></k-grid-column>
        </template>
...
export default {
  ...
  created() {
    ...
    this.initConfig(dicts => {
      ...
      if(this.dataShowTree && this.dataOpenExpandBtn) {   // 全部展开按钮值初始化
  ...
  computed: {
    ...
    tableColumnOptions() {
      return this.gridColumnOptions.filter(option => option.dataHidden == false || (option.dataHidden !== "true" &&
        option.dataHidden != true));
    },
    tableOptions() {
  ...
  methods: {
    ...
    loadActionData(params, cb) {
      ...
      if (action.indexOf("/wf/") > -1 || action.indexOf("wf/") > -1) {
        ...
        this.httpUtil
          ...
          .then(data => {
            this.list = data.rows.concat(data.rows).concat(data.rows);
    ...
    dictTransfer(dict, dictVal) {
      if (dict && dictVal) {
        let dictVals = dictVal.toString().split(",");
        let transferValue;
        for (let i = 0; i < 1; i++) {
          if (i == 0) {
