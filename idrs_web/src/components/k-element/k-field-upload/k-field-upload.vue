  ...
  export default {
    ...
    methods: {
      ...
      _dataSuccess(response, file, fileList) {
        ...
        let returnData = response.returndata;
        if(returnData!=undefined&&returnData!=null&&returnData!=''){
          if (response.success === true && returnData.type==='1') {
            Tools.alert("比对成功")
            if (this.dataSuccess) {
         ...
         if (response.success === true) {
           ...
           /*modify by zhangchangsi 20211221 添加通知超时时间，默认2500毫秒*/
           Tools.alertTime(response.returnmsg ? response.returnmsg : "上传失败", "danger", this.noticeTimeout);
            //Tools.alert(response.returnmsg?response.returnmsg:"上传失败", "danger")
            // this.doReset();
            this.dataError();
          }
