...
export default {
  ...
  methods: {
    ...
    renderPreview() {
      ...
      console.log(" renderComponent ", renderComponent, this.dynamicComponent);
      // this.$options.components['renderComponent', renderComponent]

        // <render-component></render-component>
      const component = Vue.extend(renderComponent);

      Vue.component('renderComponent', renderComponent)
      // new component().$mount("#component");
      // document.body.appendChild(_component.$el);
      // console.log("  renderComponentQQQ  ", renderComponentQQQ);
      // const renderComponentLLL = new renderComponentQQQ().$mount('.code-view');

      //在线表单时需要submitData，不然会报undefind，组件会有问题。随便给个默认值。
      return (
        <div class="code-view">
          <renderComponent submitData="submitData"></renderComponent>
        </div>
      );
    },
    // 代码检查
