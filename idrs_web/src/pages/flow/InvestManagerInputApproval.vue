  ...
  <div>
    <k-form :data-col="3" data-input-width="560px" data-label-width="180px" data-total-width="1188px" style="width: 100%">
      ...
      <k-form-item label="资金是否已全部变现并到账">
        <k-field-radio v-model="formData.is_account" data-dict="1yes0no" :data-disabled="true"/>
      </k-form-item>
      <k-form-item label="产品总资产">
        <k-field-text v-model="formData.total_assets" :data-disabled="true"/>
      </k-form-item>
    </k-form>
  </div>
</template>
...
<script>
export default {
  name: "InvestManagerInputApproval",
  props:{
    dataData:[]
  },
  data(){
