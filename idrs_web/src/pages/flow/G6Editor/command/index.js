...
class Command {
  editor = null;
  undoList = [] // 存储正向操作, 在执行 undo 的时候, pop 执行回退, 并 push 到 redoList
  redoList = []
  constructor (editor) {
    this.editor = editor
  }
  executeCommand (key, datas) {
    ...
    datas.map(data => {
      ...
      // [add | update | delete] delegate to editor.graph same method
      this.doCommand(key, model)
    })
    // trigger toolbar add event, keep (undoList and redoList sync to toolbar)
