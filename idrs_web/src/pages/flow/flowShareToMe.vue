<template>
  ...
  <div>
    ...
    <k-form-search-customize data-target="grid" v-model="queryParam">
      ...
      <k-form-item label="分享状态">
        <k-field-select v-model="queryParamStatus" data-dict="is_read" />
      </k-form-item>
      <k-form-item label="分享时间">
    ...
    <k-grid
      ...
      :data-display="false"
      data-url='/wf/businessProcess/shareProcessToMe.json'>
  ...
  computed: {
    queryParam() {
      return {
        ...
        'shareUser': this.queryParamApplyUser,
  ...
  methods: {
    ...
    getDetailList(row){
      this.httpUtil.comnUpdate({
        action: "DirecBbusinessProcess.updateShareStatus",
        params: {id : row.shareId,status:'1'},
        successAlert:false
      }).then(data => {
