...
<!--      </k-form-item>-->
      ...
      <k-form-item label="业务操作"  >
        <k-field-tree  v-model="formDat.server" data-diffcondition="id,parentId" :data-multiple="false"
                       ...
                       data-value-field="id" :data-disable-branch-nodes="true" style="width:180px !important;" />
      </k-form-item>

      <k-form-item label="审批任务名称" data-label-width="180px" >
        <k-field-text v-model="formDat.displayName"   data-display-field="name" :data-allowblank="false"
                        data-value-field="id" />
      </k-form-item>

<!--      <k-form-item label="是否在移动端显示" data-label-width="180px">
        <k-field-select v-model="formDat.appDisplay" data-dict="1yes0no" :data-allowblank="false" />
      </k-form-item>

      <k-form-item label="是否允许审批中增加节点" data-label-width="180px">
        <k-field-select v-model="formDat.addNode" data-dict="1yes0no" :data-allowblank="false" />
      </k-form-item>-->
<!--      <k-form-item label="是否绑定产品" data-label-width="180px">
        <k-field-select v-model="formDat.bindProd" data-dict="1yes0no" :data-allowblank="false"/>
      </k-form-item>-->
    </k-form><br>
    <div style="border:solid 1px" ></div><br>

        <k-form ref="prodGroupUserForm2" v-for="(item,index) in envItems" :key="index" style="display: inline-block"  :data-col="2" data-input-width="103px"
...
<!--          <k-form-item label="是否部门领导审批" data-label-width="80px" data-input-width="90px" v-if="item.nodeType!=='3'" >
    ...
    <k-form ref="prodGroupUserForm3" :data-col="5" data-input-width="120px"
      ...
      <k-form-item label="抄送人员">
        <k-field-select v-model="formDat.actorId" data-action="User.getAllUser" data-display-field="label" :data-multiple="true" :data-allowblank="true"
                        data-value-field="value" />
      </k-form-item>
      <k-form-item label="全流程推送">
<!--        <k-field-radio v-model="formDat.isPush" data-dict="1yes0no" :data-disabled="false"/>-->
        <k-field-select v-model="formDat.isPush" data-dict="1yes0no" :data-disabled="false" :data-default-value="'0'"/>
      </k-form-item>
    </k-form>

    <div style="margin: 0 auto; width: 255px">
      <k-btn class="btn-custom-primary" data-functype="SUBMIT"  :data-handler="submitHandle"
             data-target="flowProcessConfigurationAdd" :data-model="formDat">
...
<script>
  ...
  export default {
    name: "flowProcessConfigurationAdd",
    props: {
      formData:{},
    },
    ...
    methods: {
      ...
      submitHandle(value) {
        ...
        let isPush = value.isPush;
        if (isPush===undefined||isPush==''){
          isPush = '0';
        }
        // console.log("获取++++",len)
        this.httpUtil.ajax({
          url:"/wf/businessProcess/saveProcess.json" ,
          params: {server:value.server , nodeList: value.nodeList , appDisplay:value.appDisplay , actorId:value.actorId ,
            highestLevel : len  ,displayName : value.displayName , addNode : value.addNode,bindProd: value.bindProd,
            isPush:isPush},
        }).then(data => {
