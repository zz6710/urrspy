<template>
    ...
    <div class="py-page">
        <!--  动态查询框  -->
        <k-form-search-customize data-target="tableGrid" v-model="queryParam">
            ...
            <k-form-item label="结束时间">
                <k-field-date v-model="queryParamEndDateRange" data-type="daterange"
        ...
        <k-grid
                ...
                :data-display="false"
                data-url='/wf/businessProcess/queryProcessApproval.json' :data-params="{'userid':userid}">
            ...
            <template slot="operate" slot-scope="{row}">
                <k-btn class="btn-custom-text" data-descript="详情" data-functype="POPUP" data-size="mini"
                       data-target="detailPopup" :data-handler="initIsShowUrge" @click="getDetailList(row.row)">
                    详情
                </k-btn>
                <k-btn class="btn-custom-text" data-descript="撤销" data-functype="SUBMIT" data-size="mini"
                       :data-confirm="true" data-target="tableGrid" :disabled="isDisabledButton(row.row)"
                       data-url="/wf/businessProcess/revokeBusinessProcess.json">
                    撤销
                </k-btn>
                <k-btn class="btn-custom-text" data-descript="审批单下载" data-functype="POPUP" data-size="mini"
                       data-target="detailPopup2" @click="getDetailList2(row.row)">
                    审批单下载
                    <!-- 审批单下载 -->
                </k-btn>
            </template>
        ...
        <k-popup ref="detailPopup" data-title="详情" data-width="60%" >
            ...
            <div style="text-align: center;margin-top: 40px">
                <k-btn class="btn-custom-primary" data-functype="POPUP" data-descript="催办" data-size="mini" style="width: 60px"
                        :data-handler="openUrgePopup" v-show="isShowUrge">
                    催办</k-btn>
              <k-btn class="btn-custom-primary" data-functype="POPUP" data-descript="分享" data-size="mini" data-target="sharePopup" style="width: 60px">
                分享</k-btn>
...
<!--       </k-popup>-->
      ...
      <k-popup ref="sharePopup" data-title="任务分享" data-width="60%">
        <PersonTree v-bind:processData="formData" @submitClose="popupClose"/>
      </k-popup>
      <!-- 图片预览 -->
      <k-popup ref="previewImage" data-title="附件详情" data-width="60%">
        ...
        <k-popup ref="urgePopup" data-title="催办" data-width="40%">
            <k-form ref="urgeForm">
                <k-form-item label="催办人">
                    <k-field-select v-model="urgeData.solicitor" :data-data="urgeList"
                        data-display-field="operatorName" data-value-field="operator" :data-multiple="true" :data-allowblank="false"/>
                </k-form-item>
<!--                <k-form-item label="催办内容" data-col="2">-->
<!--                    <k-field-text v-model="urgeData.content" input-type="textarea" :rows="3" :data-allowblank="false"/>-->
<!--                </k-form-item>-->
                <k-form-footer data-align="center">
                    <k-btn class="btn-custom-primary" data-functype="SUBMIT"  data-from="urgeForm"
                           :data-model="urgeData" @click="submitUrge">
                        <span v-show="showSubmitBtn">确定</span>
                        <i v-show="!showSubmitBtn" class="el-icon-loading"/>
                    </k-btn>
                    <k-btn class="btn-custom-plain" data-functype="CLOSE">
...
<script>
    ...
    import KFlowUpload from "../../components/k-flow/k-flow-upload/k-flow-upload"
    ...
    export default {
        ...
        components: {
          ...
          PersonTree,
          FlowComment,
          FlowProcessInstanceListDialog,KFlowUpload,KFlowShow, VabOnlyOffice},
        props:{
            taskInfo: {},
        },
        data() {
        ...
        computed: {
            queryParam() {
                return {
                    // 'taskName': this.queryParamTaskName,
                    ...
                    'finishStartDate': this.queryParamEndDateRange ? this.queryParamEndDateRange[0] : null,
                    'finishEndDate': this.queryParamEndDateRange ? this.queryParamEndDateRange[1] : null,
                }
            },
            loadUrl() {
                const self = this;
      ...
      methods: {
          initIsShowUrge(row) {
              if (row.processStatus == '审批中') {
                  this.isShowUrge = true;
              } else {
                  this.isShowUrge = false
              }
          },
          closeEdit(){
            console.log('执行了关闭在线编辑弹框')
            this.$refs.officeEdit.closeEditor();
            this.$refs.onlinePreviewPopup.close();
          },
          //子组件关闭父组件弹窗
          popupClose() {
        ...
        fileSubmitUploadParamOfDetail(){
          ...
          this.fileData.append('processId',this.processId);
        ...
        preview(value){
          ...
          isAssetTypeAnImage(ext) {
            ...
            revokeApprovalFlow(id){
                ...
                this.httpUtil
                    ...
                    .then(res => {
                        console.log('撤销的接口数据',res)
                    });
            },
            downloadAttachment(row){
                //alert(row.contentType)
            },
            ...
            getDetailList(row){
              ...
              console.log('我发起的页面：commentParams',this.commentParams)
            ...
            renderCreateDateTime(row) {
                return Tools.formatDate(row.processInstanceCreateDate, row.processInstanceCreateTime);
            },
            ...
            isDisabledButton(row){
                if (row.creator==this.userName && row.processStatus=='审批中'){
                    return false;
                }else {
                    return true;
                }
          ...
          openUrgePopup() {
              this.httpUtil.ajax({
                  url:'/wf/approvalTask/queryApprovalSPZ.json',
                  params:{
                      processId:this.pId
                  }
              }).then(data => {
                  // let content = this.userName + '提醒您审批' + this.formData.displayName + '审批任务';
                  // this.$set(this.urgeData,'content',content);
                  let operator = '';
                  if (data.data.length >0) {
                      this.urgeList = data.data;
                      for (let i = 0;i< this.urgeList.length;i++) {
                          operator  = operator + this.urgeList[i].operator + ',';
                      }
                      operator = operator.substring(0,operator.lastIndexOf(','));
                      this.$set(this.urgeData,'solicitor',operator);
                  }
                  this.$refs.urgePopup.popup();
              })
          },
          submitUrge() {
              let result = this.$refs.urgeForm.validate();
              if (!result) {
                 return;
              }
              this.showSubmitBtn = false;
              this.$set(this.urgeData,'processId',this.pId);
              this.$set(this.urgeData,'displayName',this.formData.displayName);
              this.$set(this.urgeData,'createUser',this.userName);
              this.$set(this.urgeData,'urgeUserId',this.userid);
              this.httpUtil.ajax({
                  url:'/wf/approvalTask/urgeApproval.json',
                  params:this.urgeData
              }).then(data => {
                  this.showSubmitBtn = true;
                  Tools.alert("操作成功");
                  this.$refs.urgePopup.close();
              })
          }
        }
    };
...
<style scoped>
    ...
    /deep/ #pdfCentent .md-card .k-form-body {
