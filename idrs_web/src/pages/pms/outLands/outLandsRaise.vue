<template>
  <div>
    <div>
      <k-form-search-customize v-model="queryParam"  data-target="outLandsRaiseGrid">
        ...
        <k-btn slot="button" class="md-danger"  data-functype="POPUP" data-target="initPopup" :data-handler="clean"
               v-if="global.isShowAuthorityButton('OutLandsRaise.importOutLandsRaise')">
    ...
    <div>
      <k-grid ref="outLandsRaiseGrid" @data-row-select="selectRow" data-action="OutLandsRaise.findOutLandsRaise"  :data-autoload="true" :data-operate-column="false">
		<k-grid-column data-header="ID" data-name="id" data-hidden="true"></k-grid-column>
                  ...
                  <k-grid-column data-header="境外客户募集余额(万元)" data-name="clcBal"  data-type="money"></k-grid-column>
     ...
     <k-popup ref="initPopup" title="报送数据导入" @data-opened="uploadOpened()">
        <k-form ref="addForm" data-ui="element">
          <k-form-item label="数据日期">
            <k-field-date v-model="formData.dataDt"  data-value-format="yyyyMMdd" :data-allowblank="false"/>
          </k-form-item>
          <k-form-item label="导入sheet页">
              <k-field-text v-model="formData.sheetNumber" data-validate-type="number" data-min-value="0" :data-max-length="1" :data-allowblank="false"  data-default-value="1" />
          </k-form-item>
          ...
          <k-form-item label="报送数据导入" data-ui="element" data-input-width="500px">
            <k-field-excel-upload data-type="file" ref="uploadRef" :data-multiple="false" :data-limit='1'
                ...
                data-upload-url= "/upload/server/DpsApp/outLandsRaiseUpload.json">
            </k-field-excel-upload>
          </k-form-item>
          <k-form-footer data-align="center">
            <k-btn class="btn-custom-primary" data-functype="SUBMIT" data-target="outLandsRaiseGrid" ref="submitBtn"
...
<script>
 ...
 this.formData.dataDt='';
      ...
      onSubmitDocSuccessLegal() {
          ...
          this.$refs.outLandsRaiseGrid.load();
      },
    },
    computed:{
      queryParam(){
