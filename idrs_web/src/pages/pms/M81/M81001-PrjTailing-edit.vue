  ...
  <div>
    <k-form ref="editForm" :data-col="2">
      <k-form-item label="销售商代码" data-input-width="600px">
          <k-field-select v-model="formData.distributorCode" data-action="T8Dict.findTaDistributorInfos" :dataAllowblank="false"
                          data-display-field="distributorName"  data-value-field="distributorCode" data-disabled/>
      </k-form-item>
      <k-form-item label="产品代码" v-show="false">
          <k-field-text v-model="formData.prodCode" :dataAllowblank="false" data-disabled />
      </k-form-item>
      <k-form-item label="启用日期">
        <k-field-date v-model="formData.enableDate" data-disabled :dataAllowblank="false"/>
      </k-form-item>
      ...
      <k-form-item label="年天数">
        <k-field-select v-model="formData.tailingCommisionYeardays" data-dict="yeardays" :dataAllowblank="false"/>
      </k-form-item>
      ...
      <k-form-item label="是否累进计算" style="height: 60px">
        <k-field-select v-model="formData.graduatedCalc" data-dict="1yes0no" :dataAllowblank="false"/>
      </k-form-item>
      <k-form-item label="备注">
        <k-field-text v-model="formData.remark" data-max-value="150"/>
      </k-form-item>
    </k-form>
    ...
    <div class="my-container">
      ...
      <div class="my-item2">
        ...
        <div class="my-item2-chips">
          <md-chips v-model="moneyList" class="md-primary" style="padding-top: 4px !important;" md-placeholder="输入分段金额，并按回车确认"  md-input-type="number"  @md-delete="deleteMoney" @md-insert="insertMoney"  v-show="switchValue" ></md-chips>
        </div>
      </div>
    </div>
      ...
      <md-table v-model="tailingCommisionList">
        <md-table-row slot="md-table-row"  slot-scope="{ item }">
          ...
          <md-table-cell  md-label="费率%" >
            <md-field>
              <md-input  class="md-input" style="width: 60px; text-align: center;" v-model="item.rate"></md-input>
    ...
    <div>
      <k-form>
        <k-form-footer data-align="center">
          <k-btn data-functype="SUBMIT" data-action="T8PrjTailingCommision.updateTaPrjTailingCommision"
          data-from="addForm" :data-model="formData" data-target="T82001Grid"
                :data-handler="beforeSubmit" class="btn-custom-primary">
            <md-icon md-src="/static/svg/confirm.svg"></md-icon>确定
          </k-btn>
          <k-btn data-functype="CLOSE" class="btn-custom-plain">取消</k-btn>
        </k-form-footer>
      </k-form>
    </div>
  </div>
    ...
    mounted(){
      ...
      if(this.formData.rateType == "1"){
        ...
        this.httpUtil.comnQuery({
          action: 'TaPrjTailingCommisionList.findTaPrjTailingCommisionLists',
          params: {
            tailingCommisionCode : this.formData.tailingCommisionCode
          }
        }).then(data => {
          data.rows.forEach(e => {
            if(e.dimension1Min>0){
            ...
            this.tailingCommisionList.push({
              ...
              desc: e.dimension1Min + ' < 金额 <= ' + (e.dimension1Max == -1? '': e.dimension1Max),
              Numbermax: (e.dimension1Max == -1 ? '∞' : ''),
              rate: e.rate
              })
          })
        });
    ...
    methods: {
      beforeSubmit : function(val){
        ...
        if(isCommit){
	        ...
	        val.tailingList = this.formData.tailingList;
	        val.distributorCode = this.formData.distributorCode;
        }else{
