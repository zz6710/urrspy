    ...
    <div>
      <table class="task-table">
        <tr>
          <td>
            <div class="btn-title">
              <span style="head-span">选择产品组件</span>
            </div>
          </td>
          <td>
            <div class="btn-title" style="padding: 0px;">
              <span style="head-span">已选择组件</span>
...
<!--              <span style="font-family: PingFangSC-Regular;font-size: 12px;color: #999999;">组件顺序</span>-->
          ...
          <td>
            <ul class="add-task-ul">
              <div class="btn-bottom1">
                <div class="color-head" style="background: #4EA6EC;"></div>
                <div class="color-head-span">新增</div>
              </div>
              <li v-for="taskInfo in taskInfoMJ" :key="taskInfo.assemblyId">
                <div class="btn-bottom2" >
                  <el-button round size="small" @click="btnOnClick(taskInfo)" :title="taskInfo.assemblyDesc"
                             :class="{'btn-color-MJ' : taskInfo.inTaskSet === '1'}"> {{taskInfo.assemblyDesc}} </el-button>
                </div>
              </li>
            </ul>
          </td>
            ...
            <div class="td-div">
              <div class="cycle-div border-MJ" :style="{'height': cycleHeadHeight_MJ +'px'}">
                ...
                <div class = "cycle-detail">
                  <ul class="task-ul">
                    <draggable ref="draggableObj" v-model="prodTaskSetMJ" @update="datadragEnd" :options="{animation:500}" >
                      <transition-group>
    ...
    <div class="form-foot">
      <k-btn class="btn-custom-primary" data-functype="SUBMIT" data-action="T8ProdAssemblyList.addAssemblyTemplateList" style="margin-right: 4px;"
...
//      taskInfos: [],
  ...
  methods: {
    ...
    confirmHandler:function(params){
      ...
      params["prodTemplateCode"] = this.modValue.prodTemplateCode;
  ...
  mounted() {
    //查询产品组件，生成按钮
      this.httpUtil.comnQuery({
        action: 'T8ProdAssembly.findAssemblyTemplate',
        params: {menuItemsType : '1' ,prodTemplateCode:this.modValue.prodTemplateCode}
      }).then(data => {
    ...
    this.httpUtil.comnQuery({
      action: 'T8ProdAssembly.findAssemblyTemplate',
      params: {menuItemsType : '2',prodTemplateCode:this.modValue.prodTemplateCode}
    }).then(data => {
      for(let i=0;i<data.rows.length;i++){
        let task = data.rows[i];
    ...
    this.httpUtil.comnQuery({
      action: 'T8ProdAssembly.findAssemblyTemplate',
      params: {menuItemsType : '3',prodTemplateCode:this.modValue.prodTemplateCode}
    }).then(data => {
      for(let i=0;i<data.rows.length;i++){
        let task = data.rows[i];
    ...
    this.httpUtil.comnQuery({
      action: 'T8ProdAssembly.findAssemblyTemplate',
      params: {menuItemsType : '4',prodTemplateCode:this.modValue.prodTemplateCode}
    }).then(data => {
      for(let i=0;i<data.rows.length;i++){
        let task = data.rows[i];
    ...
    //查询该组已经配置的组件
    this.httpUtil.comnQuery({
      action: 'T8ProdAssembly.findAssemblyTemplateAdd',
      params: {prodTemplateCode:this.modValue.prodTemplateCode}
...
.task-box-MJ{
  background-image: url('/static/images/clear/task-box-mj.png');
}
.task-box-CL{
...
.box{
  ...
  z-index: 2;
  text-align: center;
  box-shadow: -1px 0px 11px -1px rgba(0,0,0,0.14);
  margin-top: 2.3px;
}
.box:before{
  content: "";
