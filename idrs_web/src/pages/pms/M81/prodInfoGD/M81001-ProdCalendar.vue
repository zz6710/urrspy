    ...
    <div class="md-layout">
      <div class="md-layout-item"
        ...
        <div ref="periodPanel" v-if="prodMode == '3'" v-for="(openItem, openItemIndex) in openRuleItems" v-bind:key="openItemIndex"
          ...
          <div class="redStar"><span>*</span></div>
          <!-- N -->
          ...
          <k-field-select v-model="T8ProdCalendar.cycleOpenType" :data-data="openRuleOptions"
                          ...
                          style="width: 70px;" :data-allowblank="prodMode != '3'"></k-field-select>

          <!-- Y 最好采用这种选择模式，否则容易出现问题，删除第一行之后会保留第一行的字典-->
          <!--          <k-field-select v-show="T8ProdCalendar.cycleOpenType == '1'" v-model="T8ProdCalendar.orderOpenDays" @data-on-change="resetOpenDate"-->
          <!--                          :data-disabled="openItem.disabled" class="md-padding-left-10"-->
          <!--                          style="width: 90px;"></k-field-select>-->
          <div class="redStar" v-show="T8ProdCalendar.cycleOpenType == '2'"><span>*</span></div>
          <div class="redStar" v-show="T8ProdCalendar.cycleOpenType == '3'"><span>*</span></div>
          <k-field-select v-show="T8ProdCalendar.cycleOpenType == '2'" v-model="T8ProdCalendar.orderOpenDays" :data-data="openWeeklyOptions" @data-on-change="resetOpenDate"
                          class="md-padding-left-10"
                          style="width: 90px;" :data-allowblank="T8ProdCalendar.cycleOpenType != '2'"></k-field-select>
          <k-field-select v-show="T8ProdCalendar.cycleOpenType == '3'" v-model="T8ProdCalendar.orderOpenDays" :data-data="openMonthlyOptions" @data-on-change="resetOpenDate"
                          class="md-padding-left-10"
                          style="width: 90px;" :data-allowblank="T8ProdCalendar.cycleOpenType != '3'"></k-field-select>
    ...
    <k-form  ref="periodInfo" :data-col="2"  dataLabelWidth="170px" dataInputWidth="300px">
      ...
      <k-form-item label="认购起始日">
        <k-field-date v-model="T8ProdCalendar.applyStartDate" data-date-format="yyyy-MM-dd"  :dataWorkday="true"
                      :dataWorkdayPgmno="this.T8ProdCalendar.pgmno" :dataMinValue="today"
                      :dataMaxValue="T8ProdCalendar.applyEndDate?T8ProdCalendar.applyEndDate:endDay"
      ...
      <k-form-item label="认购结束日">
        <k-field-date v-model="T8ProdCalendar.applyEndDate" data-date-format="yyyy-MM-dd"  :dataWorkday="true"
                      :dataWorkdayPgmno="this.T8ProdCalendar.pgmno"
                      :dataMinValue="T8ProdCalendar.applyStartDate?T8ProdCalendar.applyStartDate:today"
                      :dataMaxValue="T8ProdCalendar.establishDate?T8ProdCalendar.establishDate:endDay"
                      :dataAllowblank="false" :key="loadDate" ></k-field-date>
      </k-form-item>

      <k-form-item label="认购结束时间">
        <k-field-time v-model="T8ProdCalendar.applyEndTime" :data-allowblank="false" :data-default-value="'150000'" data-type="time"/>
      </k-form-item>
      ...
      <k-form-item label="成立日">
        <k-field-date v-model="T8ProdCalendar.establishDate" data-date-format="yyyy-MM-dd"  :dataWorkday="true"
                      ...
                      :dataWorkdayPgmno="this.T8ProdCalendar.pgmno"
                      :dataMinValue="T8ProdCalendar.applyEndDate?T8ProdCalendar.applyEndDate:today"
                      :dataMaxValue="T8ProdCalendar.openStartDate? T8ProdCalendar.openStartDate :endDay"
                      :dataAllowblank="false" :key="loadDate" ></k-field-date>
        <!--        <md-checkbox style="position:relative;left:40px;" value="1" @change="changeOpenShow"  :key="lstPeriodDate.isequalDate"   :model="lstPeriodDate.isequalDate"   v-if="this.prodMode==2||this.prodMode == 4||this.prodMode == 3"></md-checkbox>-->
      </k-form-item>
      <k-form-item label="首次开放日" v-if="this.prodMode=='1'?false:true">
        <k-field-date v-model="T8ProdCalendar.openStartDate" data-date-format="yyyy-MM-dd" :dataWorkday="true"
                      ...
                      :dataMaxValue="T8ProdCalendar.openEndDate?T8ProdCalendar.openEndDate:endDay"
      ...
      <k-form-item label="开放结束日" v-if="this.prodMode=='1'?false:true" v-show="false">
        <k-field-date v-model="T8ProdCalendar.openEndDate" data-date-format="yyyy-MM-dd" :dataWorkday="false"
                      :dataWorkdayPgmno="this.T8ProdCalendar.pgmno"
                      :dataMinValue="T8ProdCalendar.openStartDate?T8ProdCalendar.openStartDate:today"
                      :dataMaxValue="T8ProdCalendar.endDate?T8ProdCalendar.endDate:endDay"
                      :dataAllowblank="this.prodMode=='1'?true:false" :key="loadDate" ></k-field-date>
      </k-form-item>
      <k-form-item label="申赎开始时间" v-if="this.prodMode != '1'">
        <k-field-time v-model="T8ProdCalendar.redemStartTime" :data-allowblank="this.prodMode == '1'" :data-default-value="'000500'"/>
      </k-form-item>
      <k-form-item label="申赎结束时间" v-if="this.prodMode != '1'">
        <k-field-time v-model="T8ProdCalendar.redemEndTime" :data-allowblank="this.prodMode == '1'" :data-default-value="'150000'"/>
      </k-form-item>

      <k-form-item label="到期日">
        <k-field-date v-model="T8ProdCalendar.endDate" data-date-format="yyyy-MM-dd"
                      :data-on-change="getDateRegion(T8ProdCalendar.endDate,T8ProdCalendar.establishDate)"
                      :dataWorkdayPgmno="this.T8ProdCalendar.pgmno" :data-default-value="prodMode == '1'?'':'20991231'"
                      :data-min-value="T8ProdCalendar.establishDate? '('+T8ProdCalendar.establishDate : today"
                      :dataMaxValue="T8ProdCalendar.liquidate?T8ProdCalendar.liquidate:endDay"
                      :dataAllowblank="false" :key="loadDate" ></k-field-date>
      </k-form-item>
      ...
      <k-form-item label="产品期限(天)">
        <k-field-text v-model="T8ProdCalendar.productTerm" :data-allowblank="false" />
      </k-form-item>

      <k-form-item label="认购确认日">
        <k-field-select v-model="T8ProdCalendar.subscripDate" :data-allowblank="false" data-dict="t8_prod_tn" :data-default-value="'0'"/>
      </k-form-item>
      <k-form-item label="申赎确认日" v-if="this.prodMode != '1'">
        <k-field-select v-model="T8ProdCalendar.redempDate" :data-allowblank="this.prodMode == '1'"  :data-on-change="getOpenEndDate(T8ProdCalendar.endDate,T8ProdCalendar.redempDate)" data-dict="t8_prod_tn" :data-default-value="'3'"/>
      </k-form-item>
      <k-form-item label="资金到账日">
        <k-field-select v-model="T8ProdCalendar.redeemArrivalDate" :data-allowblank="this.prodMode == '1'" data-dict="t8_prod_tn"/>
      </k-form-item>
      <k-form-item label="认购计息属性" >
        <k-field-select v-model="T8ProdCalendar.subsInterestType" data-dict="t8_prod_interest_type"/>
      </k-form-item>
      <k-form-item label="认购计息年天数">
        <k-field-select v-model="T8ProdCalendar.subsInterestDays" data-dict="t8_charging_index" />
      </k-form-item>
      <k-form-item label="产品终止清算速度">
        <k-field-select v-model="T8ProdCalendar.liquidateType" :data-allowblank="false" data-dict="t8_prod_tn"/>
      </k-form-item>

      <k-form-item label="是否有封闭期投资日" v-if="this.prodMode != '1'">
        <k-field-select v-model="T8ProdCalendar.haveClosePeriod" data-dict="t8_prod_isok" :data-allowblank="this.prodMode == '1'" />
      </k-form-item>
      <k-form-item label="封闭期投资期限时间" v-if="prodMode != '1' && T8ProdCalendar.haveClosePeriod=='1'">
        <k-field-text v-model="T8ProdCalendar.closePeriodInvest" data-validate-type="money" data-type="money"
                      :data-allowblank="T8ProdCalendar.haveClosePeriod!='1'" />
      </k-form-item>
      <k-form-item label="封闭期投资期限时间单位" v-if="prodMode != '1' && T8ProdCalendar.haveClosePeriod=='1'">
        <k-field-select v-model="T8ProdCalendar.closePeriodInvestType" :data-allowblank="T8ProdCalendar.haveClosePeriod!='1'"
                        data-dict="t8_term_unit" />
      </k-form-item>

      <k-form-item label="是否有最低持有期日" v-if="this.prodMode != '1'">
        <k-field-select v-model="T8ProdCalendar.haveMinHoldingPeriodTime" data-dict="t8_prod_isok" :data-allowblank="this.prodMode == '1'"  @data-on-change="changeData" />
      </k-form-item>

      <k-form-item label="最低持有期时间" v-if="prodMode != '1' && T8ProdCalendar.haveMinHoldingPeriodTime=='1'">
        <k-field-text v-model="T8ProdCalendar.minHoldingPeriodTime" data-validate-type="money" data-type="money"
                      :data-allowblank="T8ProdCalendar.haveMinHoldingPeriodTime!='1'" />
      </k-form-item>
      <k-form-item label="最低持有期时间单位" v-if="prodMode != '1' && T8ProdCalendar.haveMinHoldingPeriodTime=='1'">
        <k-field-select v-model="T8ProdCalendar.minHoldingPeriodTimeCompany" :data-allowblank="T8ProdCalendar.haveMinHoldingPeriodTime!='1'"
                        data-dict="t8_term_unit" />
      </k-form-item>

      <k-form-item label="预约申购天数" v-if="this.prodMode == '3'">
        <k-field-text v-model="T8ProdCalendar.openPeriodDays"
                      :data-max-length="3"  data-regx="^[0-9]*$" data-regx-text="请输入整数" :data-default-value="1"/>
      </k-form-item>

      <k-form-item label="提前终止份额(份)">
        <k-field-text v-model="T8ProdCalendar.shareStop" data-type="money"  data-validate-type="money"
                      data-digits="2" :data-integer-length="14" data-show-gbmoney="true"/>
      </k-form-item>
      <k-form-item label="提前终止净值(元)">
        <k-field-text v-model="T8ProdCalendar.navStop" data-type="money"  data-validate-type="money"
                      data-digits="2" :data-integer-length="14" data-show-gbmoney="true"/>
      </k-form-item>

      <k-form-item label="募集期间资金及利息的处理方式"  :data-col="2">
        <k-field-text v-model="T8ProdCalendar.raisingPeriodType" inputType="textarea" :rows="5" :data-max-length="2000" />
      </k-form-item>
      <k-form-item label="期初观察日" v-if="this.prodMode != '1'"  :data-col="2">
        <k-field-text v-model="T8ProdCalendar.startPeriodObsDate" inputType="textarea" :rows="1" :data-max-length="2000" />
      </k-form-item>
      <k-form-item label="期末观察日" v-if="this.prodMode != '1'"  :data-col="2">
        <k-field-text v-model="T8ProdCalendar.endPeriodObsDate" inputType="textarea" :rows="1" :data-max-length="2000" />
      </k-form-item>
      <k-form-item label="封闭期说明" v-if="this.prodMode != '1'" :data-col="2">
        <k-field-text v-model="T8ProdCalendar.closePeriodDesc"  inputType="textarea" :rows="1" :data-max-length="2000"/>
      </k-form-item>
      <k-form-item label="开放日说明" v-if="this.prodMode != '1'" :data-col="2">
        <k-field-text v-model="T8ProdCalendar.periodDesc" inputType="textarea" :rows="1" :data-max-length="2000"/>
      </k-form-item>
      <k-form-item label="提前终止说明" :data-col="2">
        <k-field-text v-model="T8ProdCalendar.stopDesc"  inputType="textarea" :rows="5" :data-max-length="2000"/>
      </k-form-item>
      <k-form-footer data-align="center"  v-show="menuName == 'M81007'">
        <k-btn class="btn-custom-primary" data-functype="SUBMIT"  data-from="periodInfo" ref="submitBtn"
               :data-handler="beforeSubmit" :data-model="T8ProdCalendar" data-target=""  :data-after-success="passDataSuccess">
...
<script>
  ...
  export default {
    ...
    methods: {
      getMinDate(){
        if(this.T8ProdCalendar.applyEndDate==null && this.T8ProdCalendar.establishDate==null){
          return moment().format('YYYYMMDD');
        }else if(this.T8ProdCalendar.establishDate!=null){
          return this.T8ProdCalendar.establishDate;
        }else {
          return this.T8ProdCalendar.applyEndDate;
        }
      ...
      beforeSubmit : function(value){
        //手动验证表单
        var paramsValidate = this.$refs.periodInfo.validate()
        if (!paramsValidate) {
          return false;
        }
        this.$set(value,'assemblyMenuType','prodCalendar');
        this.$set(value,"prodMode",this.prodMode);
        if(value.cycleOpenType == '1'){
          this.$set(value,"orderOpenDays","0");
        }
        if(value.haveClosePeriod!='1'){
          this.$set(value,'closePeriodInvestType','')
        }
        ...
        if(this.prodMode == '3'){
          if(this.T8ProdCalendar.cycleOpenTerm==null||this.T8ProdCalendar.cycleOpenTerm==""){
            Tools.alert("请检选择周期开发形式,及顺延规则!","danger")
            return false;
          }
          if(this.T8ProdCalendar.cycleOpenType==null||this.T8ProdCalendar.cycleOpenType==""){
            Tools.alert("请检选择周期开发形式,及顺延规则!","danger")
            return false;
          }
          if(this.T8ProdCalendar.cycleOpenType == '2'||this.T8ProdCalendar.cycleOpenType == '3'){
            if(this.T8ProdCalendar.orderOpenDays==null||this.T8ProdCalendar.orderOpenDays==""){
              Tools.alert("请检选择周期开发形式,及顺延规则!","danger")
              return false;
            }
          }
          ...
          if(this.T8ProdCalendar.postponeRule==null||this.T8ProdCalendar.postponeRule==""){
            Tools.alert("请检选择周期开发形式,及顺延规则!","danger")
            return false;
          }
        }
        this.T8ProdCalendar.prodMode = this.prodMode;
        this.httpUtil.comnQuery({
          ...
          if(data.rows.length > 0 ){
            let row = data.rows[0];
            if (row.updFlag === '1') {
            if(row.cycleOpenTerm!==this.T8ProdCalendar.cycleOpenTerm){
              fg=false;
            }
            if(row.cycleOpenType!==this.T8ProdCalendar.cycleOpenType){
              fg=false;
            }
            if(row.orderOpenDays!==this.T8ProdCalendar.orderOpenDays){
               fg=false;
            }
            if(row.postponeRule!==this.T8ProdCalendar.postponeRule){
               fg=false;
            }
            if(row.pgmno!==this.T8ProdCalendar.pgmno){
               fg=false;
            }
            if(row.establishDate!==this.T8ProdCalendar.establishDate){
               fg=false;
            }
            if(row.openStartDate!==this.T8ProdCalendar.openStartDate){
               fg=false;
            }
            if(row.openEndDate!==this.T8ProdCalendar.openEndDate){
               fg=false;
            }
            if(row.endDate!==this.T8ProdCalendar.endDate){
               fg=false;
            }
            if(row.liquidate!==this.T8ProdCalendar.liquidate){
               fg=false;
            }
            if(row.applyStartDate!==this.T8ProdCalendar.applyStartDate){
               fg=false;
            }
            if(row.applyEndDate!==this.T8ProdCalendar.applyEndDate){
               fg=false;
            }
            if((row.redempDate!==this.T8ProdCalendar.redempDate) && this.prodMode !== '1'){
              fg=false;
            }
          }
          }
          ...
          if(!fg){
            Tools.confirm(() => {
                        ...
                        action: "T8ProdCalendar.addT8ProdCalendar",
                          params: this.T8ProdCalendar,
                          mask: false
                        }).then(res => {
                        if(res>0){
                            this.$emit('isShowButton', '1');
...
* */
      ...
      * */
      ...
      changedPeriod(item){
        ...
        this.$set(this.T8ProdCalendar,'orderOpenDays','');
      ...
      changeData(){
        if(this.T8ProdCalendar.haveMinHoldingPeriodTime=='0'){
            this.$set(this.T8ProdCalendar,'minHoldingPeriodTime','');
            this.$set(this.T8ProdCalendar,'minHoldingPeriodTimeCompany','');
        }
      },
      /**
       * 通过传过来的清盘日加载年视图，今年
      ...
      getOpenEndDate(endndDate,redempDate){
        ...
        if (endndDate) {
          ...
          this.$set(this.T8ProdCalendar,'openEndDate',openEndDate);
        }
      },
