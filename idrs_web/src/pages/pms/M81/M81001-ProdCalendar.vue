<template>
  <div v-loading="loadCalendar">
      ...
      <div class="pc-calendar">
        ...
        <div class="md-layout">
          <div class="md-layout-item"
            ...
            <div ref="periodPanel" v-show="prodMode == '4'" v-for="(openItem, openItemIndex) in openRuleItems" v-bind:key="openItemIndex"
              ...
              <!-- N -->
              <k-field-text v-model="openItem.number" class="md-padding-left-10" @data-on-change="resetOpenDate"
              ...
              <k-field-select v-show="openItem.openRule == '1'" v-model="openItem.openDate" @data-on-change="resetOpenDate"
                              :data-disabled="openItem.disabled" class="md-padding-left-10"
                              style="width: 90px;"></k-field-select>
              <k-field-select v-show="openItem.openRule == '2'" v-model="openItem.openDate" :data-data="openWeeklyOptions" @data-on-change="resetOpenDate"
                              class="md-padding-left-10"
                              style="width: 90px;"></k-field-select>
              <k-field-select v-show="openItem.openRule == '3'" v-model="openItem.openDate" :data-data="openMonthlyOptions" @data-on-change="resetOpenDate"
                              class="md-padding-left-10"
                              style="width: 90px;"></k-field-select>


              <!-- Z -->
              <k-field-select v-model="openItem.openType" :data-data="openTypeOptions" @data-on-change="resetOpenDate"
                              class="md-padding-left-10"
                              style="width: 90px;"></k-field-select>

              <span style="padding-top: 7px" >，遇节假日</span>

              <k-field-select v-model="openItem.weekendRule" :data-data="weekendRuleOptions" @data-on-change="resetOpenDate"
                              class="md-padding-left-10"
                              style="width: 160px;" ></k-field-select>

              <div class="period-icon" v-show="openRuleItems.length < 3" @click="addPeriodRow(openItem)">
                <i class="add"></i>
              </div>
              <div class="period-icon" v-show="openRuleItems.length > 1" @click="removePeriodRow(openItem)">
                <i class="substrct"></i>
              </div>
        ...
        <k-form  refs="date" :data-col="2"  dataLabelWidth="170px" dataInputWidth="300px">
          <!--<k-form-item label="日期是否相等"  v-if="this.prodMode==2||this.prodMode == 4||this.prodMode == 3">-->
          <!--<k-field-radio   :data-disabled="updateProduct" :dataAllowblank="false"  v-model="lstPeriodDate.isequalDate" data-dict="is_equal" ></k-field-radio>-->
        <!--</k-form-item>-->
          <k-form-item label="认购起始日" v-if="this.rang.subsBeginDate.showStatus">
            <k-field-date v-model="lstPeriodDate.subsBeginDate" data-date-format="yyyy-MM-dd"  :dataWorkday="true" :dataDisabled="this.rang.subsBeginDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.subsBeginDate.min"  :dataMaxValue="this.rang.subsBeginDate.max"
                          :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
          <k-form-item label="认购结束日" v-if="this.rang.subsEndDate.showStatus">
            <k-field-date v-model="lstPeriodDate.subsEndDate" data-date-format="yyyy-MM-dd"  :dataWorkday="true" :dataDisabled="this.rang.subsEndDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.subsEndDate.min"  :dataMaxValue="this.rang.subsEndDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
          <k-form-item label="成立日" v-if="this.rang.establishDate.showStatus">
            <k-field-date v-model="lstPeriodDate.establishDate" data-date-format="yyyy-MM-dd"  :dataWorkday="true" :dataDisabled="this.rang.establishDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.establishDate.min"  :dataMaxValue="this.rang.establishDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          ...
          <k-form-item label="起息日" v-if="this.rang.valueDate.showStatus">
            <k-field-date v-model="lstPeriodDate.valueDate" data-date-format="yyyy-MM-dd" :dataWorkday="true" :dataDisabled="this.rang.valueDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.valueDate.min"  :dataMaxValue="this.rang.valueDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
          <k-form-item label="开放开始日" v-if="this.rang.openBeginDate.showStatus">
            <k-field-date v-model="lstPeriodDate.openBeginDate" data-date-format="yyyy-MM-dd" :dataWorkday="true" :dataDisabled="this.rang.openBeginDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.openBeginDate.min"  :dataMaxValue="this.rang.openBeginDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
          <k-form-item label="开放结束日" v-if="this.rang.openEndDate.showStatus">
            <k-field-date v-model="lstPeriodDate.openEndDate" data-date-format="yyyy-MM-dd" :dataWorkday="true" :dataDisabled="this.rang.openEndDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.openEndDate.min"  :dataMaxValue="this.rang.openEndDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
          <k-form-item label="到期日" v-if="this.rang.endDate.showStatus">
            <k-field-date v-model="lstPeriodDate.endDate" data-date-format="yyyy-MM-dd" :dataWorkday="true" :dataDisabled="this.rang.endDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.endDate.min"  :dataMaxValue="this.rang.endDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
          <k-form-item label="清盘日" v-if="this.rang.windingDate.showStatus">
            <k-field-date v-model="lstPeriodDate.windingDate" data-date-format="yyyy-MM-dd" :dataWorkday="true" :dataDisabled="this.rang.windingDate.disabled"
                          :dataWorkdayPgmno="this.pgmno" :dataMinValue="this.rang.windingDate.min"  :dataMaxValue="this.rang.windingDate.max"
                         :dataAllowblank="false" :key="loadDate" ></k-field-date>
          </k-form-item>
        </k-form>
        ...
        <div class="md-layout">
          ...
          <!--日期样式图标 start-->
          <!--<div class="md-layout-item"-->
               <!--style="margin-top: 14px; margin-bottom: 5px; display: flex; width: 100%; justify-content: flex-start; align-items: center;">-->

            <!--<div class="md-padding-right-8" style="display:flex; align-items: center;" v-for="(legend, legendIndex) in legendItems" v-bind:key="legendIndex">-->
              <!--<div class="pic-cycle" :class="legend.legendClass"></div> {{legend.name}}-->
            <!--</div>-->
          <!--</div>-->
          <!--日期样式图标 end-->
          <div class="md-layout-item" v-show="showCalendarFlag"
               style="margin-top: 14px; margin-bottom: 5px; display: flex; width: 100%; justify-content: flex-end; align-items: center;">
            ...
            <div class="fc-button-group">
              ...
              <span class="pdc-year-header">&nbsp;{{currentYear}}&nbsp;</span>
              <span class="pdc-year-text">年</span>
    ...
    <table class="year-table" cellspacing="0" cellpadding="0" style="padding-left: 0px;padding-right: 50px; width: 1060px;" v-show="showCalendarFlag" >

      <div class="year-calendar" v-for="(_month,index) in MONTHS[currentYear]" :key="index">

        <tbody style="display: inline">
        <div class="pcy-month">
          <span class="month-header">{{ index }} 月</span>
        </div>

        <tr class="year-month">
          <td :class="weekly.itemClass" v-for="(weekly, weeklyIndex) in weeklyItems" v-bind:key="weeklyIndex">
            {{weekly.name}}
          </td>
        </tr>
        ...
        <tr class="year-month" v-for="(week, weekIndex) in _month" v-bind:key="weekIndex">
          <td v-for="(date, dateIndex) in _month[weekIndex]"
              ...
              @mouseenter="date.disable ? '' : mouseOnDay(date) "
              @mouseleave="date.disable ? '' : mouseOutDay(date)"
              style="position:relative"
          >

            <!-- 显示日期 -->
            <div :key="date.dayID" class="year-date" :id="date.dayID" @click="clickWorkday(date)"
                 :class="[date.class, date.isAlive, date.calendarStyle]">
              <div :class="date.top">{{date.day}}</div>

              <div v-show="date.isAlive != null && date.wholeDay == showTipsDate" class="fc-popover fc-more-popover "
                   :class="[date.class]"
                   style="margin-top: -2px; margin-left: 19px;">
                <div class="fc-header fc-widget-header">
                  <span class="fc-title">{{date.name}}</span>
                </div>
              </div>
...
* */
    ...
    watch: {
      ...
      pgmno: function(){
        //console.log("watch pgmno value is: " + this.pgmno);
        this.initData ();
