  ...
  <div>
    ...
    <k-form class="my-form" ref="performanceInfo" :data-col="2" dataLabelWidth="170px" dataInputWidth="300px">
      ...
      <k-form-item label="t8ProdInfoId" v-show="false">
        <k-field-text v-model="T8ProdPerformance.t8ProdInfoId" :data-disabled="true"/>
      </k-form-item>
      <k-form-item label="prodCode" v-show="false">
        <k-field-text v-model="T8ProdPerformance.prodCode" :data-disabled="true"/>
      </k-form-item>
      <k-form-item label="基准类型">
        <k-field-select v-model="T8ProdPerformance.baseType" data-dict="t8_base_type" :data-disabled="true" @data-on-change="inlitFrom" :dataAllowblank="false"/>
      </k-form-item>
      <div class="k-form-body" v-if="this.isShowTable && T8ProdPerformance.baseType != '0'">

      <k-form-item label="基准利率%" v-show="T8ProdPerformance.baseType == '1'" v-if="!showShareSort">
        ...
        <k-form-item label="基准利率下限%" v-if="!showShareSort&&T8ProdPerformance.baseType == '2'">
        ...
        <k-form-item label="基准利率上限%" v-if="T8ProdPerformance.baseType == '2' &&!showShareSort">
        ...
        <k-form-item label="市场利率类型" v-if="T8ProdPerformance.baseType == '4'&&!showShareSort">
        ...
        <k-form ref="addForm2" v-for="(item,index) in envItems" :key="index"
                v-if="(T8ProdPerformance.baseType == '3' || T8ProdPerformance.baseType == '5') && !showShareSort"
                :data-col="6" data-input-width="120px" data-label-width="100px" data-total-width="1300px">
          <div class="md-addForm2" style="display: flex;justify-content: space-evenly;align-items: center;text-align: left;float: left">
            <k-form-item :label="index=='0'?'':'操作符'" >
              <k-field-select v-model="item.operator" data-dict="t8_performance_operator" data-display-field="itemval"  data-value-field="itemkey"
                              :data-allowblank="false" :data-disabled="true"></k-field-select>
            </k-form-item>
            ...
            <k-form-item :label="'指数名称'" v-if="item.ratioType == '1' || item.ratioType == null" key="ratioIndex">
            ...
            <k-form-item :label="'自定义利率'" v-if="item.ratioType == '2'" key="fixedValue">
              <k-field-text v-model="item.fixedValue" :data-allowblank="false" :data-disabled="true" :data-max-length="8"
            ...
            <k-form-item :label="'市场利率'" v-if="item.ratioType == '3'" key="marketRate">
              <k-field-select v-model="item.marketRate" :data-allowblank="false" :data-disabled="true"
                            data-dict="t8_market_rate" data-display-field="itemval"  data-value-field="itemkey"/>
            </k-form-item>
            <k-form-item :label="'系数%'">
              <k-field-text v-model="item.coefficient" :data-allowblank="false" :data-disabled="true" :data-max-length="8"
                            data-min-value="0" data-max-value="100" data-integer-length="3" data-validate-type="money"
                            data-type="money" data-digits="2"/>
            </k-form-item>
          </div>
      ...
      <k-form-item label="是否存在超额业绩报酬" >
        <k-field-radio v-model="T8ProdPerformance.excessStatus" :data-disabled="true" :data-data="options" data-value-field="value" data-display-field="label"  />
      </k-form-item>
        <k-form-item label="业绩报酬提取比例%" v-show="T8ProdPerformance.excessStatus == 'true'">
          <k-field-text v-model="T8ProdPerformance.performanceOut" :data-max-length="8" :data-disabled="true"
                        data-min-value="0" data-integer-length="3" data-validate-type="money" data-max-value="100"
                        data-type="money" data-digits="2" :data-allowblank="false"/>
        </k-form-item>
<!--      <k-form-item label="自定义利率%" v-if="!showShareSort">-->
<!--        <k-field-text v-model="T8ProdPerformance.custom" :data-max-length="8" :data-disabled="true"-->
<!--                      data-min-value="0" data-integer-length="3" data-validate-type="money"-->
<!--                      data-type="money" data-digits="2"/>-->
<!--      </k-form-item>-->

      <!--      <k-form-item label="计提比例%" v-show="!switchSegmentValue  && T8ProdPerformance.baseType == '2'">-->
      <!--        <k-field-text v-model="T8ProdPerformance.indexProfit" :data-max-length="8" :data-disabled="true"-->
      <!--                      data-min-value="0" data-integer-length="3" data-validate-type="money"-->
      <!--                      data-type="money" data-digits="2"/>-->
      <!--      </k-form-item>-->

        <k-form-item label="超额业绩报酬（产品要素表格使用）" :data-col="2" v-show="T8ProdPerformance.excessStatus == 'true'">
          <k-field-text v-model="T8ProdPerformance.excessPerfExplain" :data-max-length="2000" inputType="textarea"
                        :rows="5" :data-disabled="true" :data-allowblank="false"/>
        </k-form-item>
      <k-form-item label="超额业绩报酬（产品费用及税收使用）" :data-col="2" v-show="T8ProdPerformance.excessStatus == 'true'">
        <k-field-text v-model="T8ProdPerformance.excessPerfMethod" :data-max-length="2000" inputType="textarea"
                      :rows="5" :data-disabled="true" :data-allowblank="false"/>
      </k-form-item>
      <k-form-item label="超额业绩报酬计提基准" :data-col="2" v-show="T8ProdPerformance.excessStatus == 'true'">
        <k-field-text v-model="T8ProdPerformance.performanceProvision" :data-max-length="2000" inputType="textarea"
                      :rows="5" :data-disabled="true" :dataAllowblank="false"/>
      </k-form-item>

      <k-form-item label="业绩比较基准（产品要素、投资管理使用）" :data-col="2" v-if="!showShareSort" :hidden="T8ProdPerformance.baseType == '0' || T8ProdPerformance.baseType == '1' || T8ProdPerformance.baseType == '2'">
    ...
    <div class="my-container" v-show="T8ProdPerformance.baseType == '2'&&!showShareSort">
      <div class="my-item2">
        <div style="width: 135px !important">
          <md-switch v-model="switchSegmentValue" class="md-info" @change="changeSegmentType" :disabled="true">分段计提
          </md-switch>
        </div>
        <!--        <div class="my-item2-chips" >-->
        <!--          <md-chips v-model="moneyList" class="md-primary" style="padding-top: 4px !important;"-->
        <!--                    md-input-type="number" :disabled="true" v-show="switchSegmentValue && T8ProdPerformance.baseType == '2'" ></md-chips>-->
        <!--        </div>-->
      </div>
    </div>

    <div label="分段计提展示" class="my-table"
         v-show="switchSegmentValue  && T8ProdPerformance.baseType == '2'&&!showShareSort">
      <md-table>
        <md-table-row>
          <md-table-head v-show="moneyList.length > 0  && T8ProdPerformance.baseType == '2'" :data-disabled="true">
            基准利率区间%
          </md-table-head>
          <md-table-head v-show="moneyList.length > 0  && T8ProdPerformance.baseType == '2'" :data-disabled="true">
            {{ feeTableHead }}
        ...
        <md-table-row v-show="timeList.length > 0 || moneyList.length > 0 && T8ProdPerformance.baseType == '2'"
                      v-for="(item,index) in tailingCommisionList" :key="index" :disabled="true">
          <md-table-cell v-show="item.showMoneyTd && T8ProdPerformance.baseType == '2'" :rowspan="item.moneyRowspan"
                         :disabled="true">
            {{ item.moneyDesc }}
          </md-table-cell>
          <md-table-cell v-show="item.showTimeTd && T8ProdPerformance.baseType == '2'" :disabled="true">
            {{ item.timeDesc }}
          </md-table-cell>
          <md-table-cell>
            <md-field>
              <k-field-display class="title-desc" style="width: 80px;"
                            v-model="item.rate"></k-field-display>
...
<!--                          :data-allowblank="false"></k-field-select>-->
<!--        </k-form-item>-->
<!--        <k-form-item :label="'指数名称'" v-if="item.ratioType == '1' || item.ratioType == null">-->
<!--          <k-field-select v-model="item.ratioIndex" data-value-field="indexCode" :data-allowblank="false" :data-disabled="true"-->
<!--                          data-display-field="indexName" data-action="T8IndexInfo.find"/>-->
<!--        </k-form-item>-->
...
<!--          <k-field-text v-model="item.marketRate" :data-allowblank="false" :data-disabled="true"-->
<!--                        data-dict="t8_market_rate" data-display-field="itemval"  data-value-field="itemkey"/>-->
<!--        </k-form-item>-->
<!--        <k-form-item :label="'系数%'">-->
<!--          <k-field-text v-model="item.coefficient" :data-allowblank="false" :data-disabled="true" :data-max-length="8"-->
...
<script>
  ...
  export default {
    computed: {
      switchSegmentValue2() {
        if('excessStatus' in this.T8ProdPerformance){
        }else {
          this.T8ProdPerformance.excessStatus=true
        }
    ...
    methods: {
      ...
      submitHandle(value) {
        //验证
        let result = true;
        result = this.$refs.performanceInfo.validate();
    ...
    created() {
      //console.log("hhhh",this.T8ProdPerformance)
