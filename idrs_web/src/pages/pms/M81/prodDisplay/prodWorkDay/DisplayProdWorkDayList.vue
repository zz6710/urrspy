    ...
    <div class="top">
      <div class="options">
        <div style="display: inline-block;margin-right: 32px;">
          <span class="circle" style="background-color:#41A0EB;border:1px solid rgb(114, 214, 169)"> &nbsp;</span>
          <small style="font-size:12px">产品工作日</small>
...
<!--          <k-field-text v-model="currentYear" :data-data="yearSelectList" style="width: 100px;"-->
<!--                          data-display-field="label" data-value-field="value"-->
<!--                          @data-on-change="refreshData"></k-field-text>-->
    ...
    <!--//循环月份-->
    <div id="calendar" v-for="(item,index) in MONTHS" :key="index" style="margin-top: 40px">
      <div class="month">
        <ul>
          <li class="year-month">
      ...
      <ul class="days">
        ...
        <li v-for="(day,i) in item" :key="i" >
          <el-checkbox-group v-model="checkList" size="mini" >
            <div class="not-current-month">
              <el-checkbox-button @change="changeWorkdays(day,i,index)" :class="isWeekend(i)" ref="dayButton"
                                  v-if="day.getMonth()+1 == index" :label="JSON.stringify(day).substr(1,10)"
                                  :disabled="day > prodOpenEnd || day < prodOpenStart || day < today
                                 || day < minWorkday || day > maxWorkday" disabled>
                {{day.getDate()}}
              </el-checkbox-button>
  ...
  export default {
    ...
    data: function() {
      return {
        ...
        label: [],
        copyLabel: [],
        prodOpenStart:new Date(),
        prodOpenEnd:new Date(),
        dateDisabled:true,
        today: new Date(),
        maxWorkday: new Date(),//系统工作日最大值
        minWorkday: new Date(),//系统工作日最小值
        deleteList:[],
        addList:[],
        changeList:[],
      };
    ...
    watch:{
      ...
      fromData(value){
        this.pgmno = value.pgmno;
        this.openStartDate = value.openStartDate;
        this.openEndDate = value.openEndDate;
        this.prodInfoId = value.prodInfoId;
        this.prodCode = value.prodCode;
        this.prodOpenStart = new Date(this.formatSelectNumber(this.openStartDate));
        this.prodOpenEnd = new Date(this.formatSelectNumber(this.openEndDate));
        if (this.pgmno) {
          this.listWorkdays();//查询当年该工作日方案的所有日期
          this.initData(null); //初始化日期表格
          let listStrDate = value.changeDate.substring(0,value.changeDate.length-1);
          let listDate = listStrDate.split(',');
          listDate.forEach(selectDay => {
            selectDay = this.formatSelectNumber(selectDay);
            this.changeList.push(selectDay);
          });
          this.listProdWorkdays();//查询产品现有开放日
   //       this.initYearSelectList();//前十年和后十年
        }
      }
    },
    created() {
    ...
    methods: {
      ...
      listWorkdays(){//查询系统工作日
        this.loading = true;
        this.httpUtil
          .comnQuery({
            action: "WorkdayItem.findMaxMin",
            params: {"pgmno": this.pgmno, "workday":this.currentYear}
          }).then(result => {
          this.minWorkday = new Date(this.formatSelectNumber(result.rows[0].minWorkday));
          this.maxWorkday = new Date(this.formatSelectNumber(result.rows[0].maxWorkday));
          // result.rows.forEach(selectDay => {
          //   selectDay.workday = this.formatSelectNumber(selectDay.workday);
          //   this.workdayList.push(selectDay.workday);
          // });
          setTimeout(() => {
            this.loading = false;
          }, 500);
      ...
      listProdWorkdays() {
        ...
        this.httpUtil.comnQuery({
          action: "T8ProdWorkdays.findT8ProdWorkdayss",
          params: {"prodCode":this.prodCode , "dateType":"1"}
        }).then(result => {
          this.workdays.data = result.rows
          this.workdays.data.forEach(selectDay => {
            selectDay.changeDate = this.formatSelectNumber(selectDay.changeDate);
            this.checkList.push(selectDay.changeDate);
            this.aa.push(selectDay.changeDate);
          });

          let s1 = new Set(this.changeList);
          let s2 = new Set(this.checkList);
          this.addList = this.changeList.filter( x => !s2.has(x) );
          this.deleteList = this.checkList.filter( x => !s1.has(x) );


          setTimeout(() => {
            this.$nextTick(()=>{
              this.addList.forEach(selectDay => {
                this.changeWorkdays(selectDay);
              });
              this.deleteList.forEach(selectDay => {
                this.changeWorkdays(selectDay);
              });
            });
            this.loading = false;
      ...
      initData: function(cur) {
        ...
        this.months.forEach(month => {
          ...
          var date = new Date(d);
          this.currentDay = date.getDate();   //1号
          this.currentYear = date.getFullYear();  //年

          this.currentMonth = date.getMonth() + 1; //月份
          this.currentWeek = date.getDay(); // 1...6,0 一号是周几
      ...
      saveWorkdays() {
        ...
        this.httpUtil.comnUpdate({
          action: "T8ProdWorkdays.upAddT8ProdWorkdays",
          // TODO
          params: {"t8ProdInfoId": String(this.prodInfoId),
            "prodCode": String(this.prodCode),
            "changeDate": this.action.workdays,
            "dateType":"1","adjustReason":this.fromData.adjustReason},
          mask: true,
      ...
      changeWorkdays(day, i, index) {
        if (this.aa.indexOf(day) > -1) {
          let label = document.getElementsByClassName("el-checkbox-button--mini");

          for (let i = 0; i < label.length; i++) {
            if (label[i].firstChild.value == day) {
