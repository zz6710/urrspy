  ...
  <div>
    <k-form ref="editForm" :data-col="2">
      <k-form-item label="交易费用类型">
        <k-field-display v-model="T8FeeDealDiscount.feeTypeDeal" data-dict="t8_fee_type_deal"/>
      </k-form-item>
      <!-- <k-form-item label="交易客户类型">
        <k-field-display v-model="T8FeeDealDiscount.custTypeDeal" data-dict="t8_cust_type_deal"/>
      </k-form-item> -->
      <k-form-item label="优惠比例（%）" v-show="this.isShowDiscountFee">
        <k-field-display v-model="T8FeeDealDiscount.discountProportion" data-validate-type="number" data-digits="2"
                         data-min-value="0" data-max-value="100" :dataAllowblank="!this.isShowDiscountFee"/>
      </k-form-item>
      <k-form-item label="优惠生效日期">
        <k-field-display v-model="T8FeeDealDiscount.validateDate"
                         data-type="date"
                         ref="startDate"
        />
      </k-form-item>
      <k-form-item label="优惠失效日期">
        <k-field-display v-model="T8FeeDealDiscount.invalidateDate"
                      data-type="date"/>
      </k-form-item>
      <k-form-item label="基本费率(%)" v-show="!(this.switchSegmentValue||this.switchTimeValue)">
        <k-field-display v-model="T8FeeDealDiscount.baseFeeRate" :data-max-length="8" data-integer-length="3" data-max-value="100"
                      data-digits="4"
      ...
      <k-form-item label="优惠后费率(%)" v-show="!(this.switchSegmentValue||this.switchTimeValue)">
        <k-field-display v-model="T8FeeDealDiscount.feeRateAfterDiscount" :data-max-length="8" data-integer-length="3" data-max-value="100"
                      data-digits="4"
                      data-min-value="0" data-validate-type="money" data-type="money"  data-disabled="true"/>
      </k-form-item>
      <k-form-item label="决策类型">
        <k-field-display v-model="T8FeeDealDiscount.decisionType"
                        data-dict="decision_type" @data-on-change="changeDecisionType"
        ></k-field-display>
      </k-form-item>
      <k-form-item label="会议/审批单">
        <k-field-select v-model="T8FeeDealDiscount.meetingId"
                         :data-params="{type:T8FeeDealDiscount.decisionType,prodCode:T8FeeDealDiscount.prodCode}"
                         data-action="QuotaMeeting.findMeetByTypeAndCode"
                         data-display-field="meetingName"
                         data-value-field="id"
                         ref="meetings" :data-disabled="true"
        ></k-field-select>
      </k-form-item>
    </k-form>
        ...
        <md-table-row v-show="timeList.length > 0 || moneyList.length > 0"  v-for="(item,index) in tailingCommisionList" :key="index">
          ...
          <md-table-cell>
            <md-field style="width:70%;">
              ...
              <md-input class="md-input" style="text-align: center;width:60px;" v-model="item.amtRate" maxlength="3" md-input-type="number" :disabled="true" v-show="ProdFeeDeal3.chargeType=='1'"></md-input>
    ...
    <div>
      <k-form>
        <k-form-footer data-align="center">
          <k-btn class="btn-custom-primary" data-functype="SUBMIT" data-action="T8FeeDealDiscount.confirmDiscount" data-from="editForm"
...
export default {
  ...
  created() {
    this.T8FeeDealDiscount=this.info
    this.feeId = this.info.id;
    ...
    //查询交易费用信息分段信息
    if(this.info.isAmtSegment == 1 || this.info.isTimeSegment == 1){
      if (this.info.isAmtSegment == 1) {
        this.switchSegmentValue = true;
      }
      if (this.info.isTimeSegment == 1) {
        this.switchTimeValue = true;
      }
      this.findPrjFeeListById();
    }
    this.changeDecisionType();

    this.$set(this.T8FeeDealDiscount,'meetingId',this.T8FeeDealDiscount.meetingId);
    //根据收费类型改变描述
  ...
  watch:{
    'T8FeeDealDiscount.discountProportion':function(value){
      if(value!=''&&value!=0){
        this.T8FeeDealDiscount.feeRateAfterDiscount = this.T8FeeDealDiscount.baseFeeRate*(1-(value/100));
        if(this.tailingCommisionList != []){
          for(let i=0;i<this.tailingCommisionList.length;i++){
            this.tailingCommisionList[i].amtRateAfterDiscount = this.tailingCommisionList[i].amtRate*(1-(value/100));
          }
        }
      }
    ...
    'T8FeeDealDiscount.baseFeeRate':function(value){
      if(this.T8FeeDealDiscount.discountProportion!=0){
        this.T8FeeDealDiscount.feeRateAfterDiscount = value*(1-(this.T8FeeDealDiscount.discountProportion/100))
      }
    }
  },
  methods: {
    ...
    initTable : function(rows){
      ...
      rows.forEach((e,index) => {
        if(!(e.minHoldDays == null || e.minHoldDays == '')){
          if(e.minHoldDays != "0" && (this.timeList.indexOf(e.minHoldDays) === -1)){
            this.timeList.push(e.minHoldDays);
          }
          this.switchTimeValue = true;
        }else{
      ...
      rows.forEach((e,index) => {
        ...
        if(e.maxAmt != "0"&&e.maxAmt !="0.00"&&e.maxAmt !=""&&e.maxAmt !=null){  //本段最大金额不为0
          showMoneyFlag = true;
        }
        if(showMoneyFlag){
          this.tailingCommisionList.push({
            ...
            minAmt: e.minAmt,
            maxAmt: e.maxAmt,
            minHoldDays: e.minHoldDays,
            maxHoldDays: e.maxHoldDays,
            moneyDesc: e.minAmt == null ? null :(e.minAmt + ' <= 金额 < ' + (e.maxAmt == -1 ? '∞' : e.maxAmt)),
            //timeDesc: e.dimension2Min == null ? null :(e.dimension2Min + ' <= 天数 < ' + (e.dimension2Max == -1 ? '∞' : e.dimension2Max)),
            discountRate:e.discountRate,
            amtRate: e.amtRate,
            moneyRowspan: e.maxHoldDays == "0" ? this.timeList.length+1 : 1,    //2维的时候金额分段只要一条，所以这里选第一条来占行数，后面的都不展示
            showMoneyTd:  showMoneyFlag,
            showTimeTd: (e.maxHoldDays == null||e.maxHoldDays==0) ? false : true,
            amtRateAfterDiscount:e.amtRateAfterDiscount,
          })
        }else{
          this.tailingCommisionList.push({
            minAmt: e.minAmt,
            ...
            timeDesc: e.minHoldDays == null ? null :(e.minHoldDays + ' <= 天数 < ' + (e.maxHoldDays == -1 ? '∞' : e.maxHoldDays)),
            discountRate:e.discountRate,
            amtRate: e.amtRate,
            moneyRowspan: e.maxHoldDays == "0" ? this.timeList.length+1 : 1,    //2维的时候金额分段只要一条，所以这里选第一条来占行数，后面的都不展示
            showMoneyTd:  showMoneyFlag,
    ...
    //决策类型改变事件
    changeDecisionType() {
      //this.T8FeeDealDiscount.meetingId = '';
      this.$refs.meetings.load({type: this.T8FeeDealDiscount.decisionType, prodCode: this.T8FeeDealDiscount.prodCode});
    },
    //根据交易费用id查询分段信息
    findPrjFeeListById(){
      this.httpUtil.comnQuery({
        action: 'T8FeeDealDiscountDetail.findFeeDealDiscountDeatil',
        params: {
        ...
        if(data.rows.length<2){
          ...
          data.rows.forEach((e,index) => {
            if(e.maxHoldDays != "0"&&e.maxHoldDays != "0.00"&&e.maxHoldDays != ""){
              if(index<data.rows.length-1){
                this.timeList.push(e.maxHoldDays);
              }

            }else if(e.maxAmt != "0"&&e.maxAmt != "0.00"&&e.maxAmt != ""){
              if(index<data.rows.length-1) {
                this.moneyList.push(e.maxAmt);
              }
            }
          });
    ...
    beforeSubmit : function(value){
      ...
      this.$set(value,'feeTableDiscount',this.feeTableDiscount);
