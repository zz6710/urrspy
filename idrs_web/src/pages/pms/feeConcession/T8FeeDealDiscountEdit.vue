  ...
  <div>
    <k-form ref="editForm" :data-col="2">
      ...
      <k-form-item label="决策类型">
        <k-field-select v-model="T8FeeDealDiscount.decisionType"
                        data-dict="decision_type"
                        @data-on-change="changeDecisionType"
        ></k-field-select>
      </k-form-item>
      <k-form-item label="会议/审批单">
        <k-field-select v-model="T8FeeDealDiscount.meetingId"
                        ...
                        data-action="QuotaMeeting.findMeetByTypeAndCode" data-display-field="meetingName"
                        data-value-field="id" ref="meetings"
        ></k-field-select>
      </k-form-item>
        ...
        <md-table-row v-show="timeList.length > 0 || moneyList.length > 0"  v-for="(item,index) in tailingCommisionList" :key="index">
          ...
          <md-table-cell>
            <md-field style="width:70%;text-align:center;" >
              <span class="md-suffix" v-if="ProdFeeDeal3.chargeType=='0'||ProdFeeDeal3.chargeType==''">￥</span>
              <md-input class="" style="text-align: center;width:60px;padding-right:0px !important;" v-model="item.discountRate"  data-max-value="100"
    ...
    <div>
      <k-form>
        <k-form-footer data-align="center">
          <k-btn class="btn-custom-primary" data-functype="SUBMIT" data-action="T8FeeDealDiscount.updateDealDiscount" data-from="editForm"
  ...
  methods: {
    ...
    initTable: function (rows) {
      ...
      rows.forEach((e,index) => {
        ...
        if(e.maxAmt != "0"&&e.maxAmt !="0.00"&&e.maxAmt !=null&&e.maxAmt !=''){  //本段最大金额不为0
          showMoneyFlag = true;
        }
        if(showMoneyFlag){
