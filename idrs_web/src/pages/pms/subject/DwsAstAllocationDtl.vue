	...
	<div class="py-page">
		<div>
			<k-form-search-customize
				ref="searchRef"
				data-model-name="DwsAstAllocationDtl"
				data-label-width="80px"
				v-model="searchParam"
				data-target="dwsAstAllocationDtlGrid"
			>
		...
		<div class="py-page-container">
			...
			<k-grid
				ref="dwsAstAllocationDtlGrid"
				@data-row-select="selectRow"
				data-action="DwsAstAllocationDtl.findDwsAstAllocationDtls"
				:data-autoload="false"
				data-dict-type="1"
			>
				...
				<k-grid-column data-header="是否债务证券（金融）" data-name="finDebSec" data-dict="1yes2no" data-width="100"></k-grid-column>
				<k-grid-column data-header="是否债务证券（非金融）" data-name="nonFinDebSec" data-dict="1yes2no" data-width="100"></k-grid-column>
				<k-grid-column data-header="评级" data-name="rat" data-dict="g06_bond_credit" data-dict-type="0" data-width="220"></k-grid-column>
				<k-grid-column data-header="非金融企业债分类" data-name="nonFinLab" data-dict="fjrqyzfl"></k-grid-column>
				<k-grid-column data-header="银行资本补充工具分类" data-name="bankCapSup" data-dict="yhzbbcgjfl" data-width="130"></k-grid-column>
				<k-grid-column data-header="市值" data-name="mktVol"></k-grid-column>
				<template slot="operate" slot-scope="scope">
					<k-btn
						class="btn-custom-text"
						data-descript="修改资产配置情况明细表"
						data-functype="POPUP"
						data-size="mini"
						data-target="editDwsAstAllocationDtlPopup"
					>
						修改
					</k-btn>
					<k-btn
						...
						data-action="DwsAstAllocationDtl.deleteDwsAstAllocationDtl"
						data-size="mini"
						data-type="danger"
						data-target="dwsAstAllocationDtlGrid"
						:data-confirm="true"
						data-descript="删除资产配置情况明细表"
					>
						删除
					</k-btn>
				</template>
		...
		<!--    修改资产配置情况明细表弹出框   -->
		<k-popup ref="editDwsAstAllocationDtlPopup" data-title="修改">
			<k-form ref="editDwsAstAllocationDtlForm" :data-col="2" data-label-width="170px">
				...
				<k-form-item label="是否债务证券（金融）" :class="[handleItemDiff('finDebSec')]">
					<k-field-select v-model="formData.finDebSec" data-dict="1yes2no" data-dict-type="1" />
				</k-form-item>
				<k-form-item label="是否债务证券（非金融）" :class="[handleItemDiff('nonFinDebSec')]">
					<k-field-select v-model="formData.nonFinDebSec" data-dict="1yes2no" data-dict-type="1" />
				</k-form-item>
				<k-form-item label="评级" :class="[handleItemDiff('rat')]">
					<k-field-select v-model="formData.rat" data-dict="g06_bond_credit" />
				</k-form-item>
				<k-form-item label="非金融企业债分类" :class="[handleItemDiff('nonFinLab')]">
					<k-field-select v-model="formData.nonFinLab" data-dict="fjrqyzfl" data-dict-type="1" />
				</k-form-item>
				<k-form-item label="银行资本补充工具分类" :class="[handleItemDiff('bankCapSup')]">
					<k-field-select v-model="formData.bankCapSup" data-dict="yhzbbcgjfl" data-dict-type="1" />
				</k-form-item>
				...
				<k-form-footer data-align="center">
					<k-btn
						...
						data-functype="SUBMIT"
						data-action="DwsAstAllocationDtl.updateDwsAstAllocationDtl"
						data-from="editDwsAstAllocationDtlForm"
						:data-model="formData"
						data-target="dwsAstAllocationDtlGrid"
			...
			<k-form ref="addNewForm" data-ui="element">
				...
				<k-form-item label="附件" data-ui="element" data-input-width="500px">
					<k-field-excel-upload
						...
						data-upload-url="upload/server/RptApp/uploadDwsAstAllocationDtl.json"
						data-tip-succ="导入成功"
					>
					</k-field-excel-upload>
				</k-form-item>
		...
		<!-- <GenerateFormAgainDialog ref="formAgainRef" paraid="90000052008" menuId="M061818" buttonName="重新生成报表" /> -->
...
export default {
	...
	methods: {
		...
		// 				async: true,
		// 				params: { menuId: "M061818", buttonName: "重新生成报表", reportDate: this.lastDay, paraid: "90000052008" },
		// 				successAlert: false,
		...
		onSubmitNewSuccess() {
			if (this.searchParam.reportDate) {
				this.$refs.dwsAstAllocationDtlGrid.load(this.searchParam);
			}
		...
		handleBefore() {
			if (this.formNoChangeCb()) {
				this.$refs.editDwsAstAllocationDtlPopup.close();
				return false;
			}
			return true;
		},
