<template>
  <div>
    <div>
      <k-form-search-customize v-model="searchParam" data-target="GridAffiliateOtherAnalysisGrid">
      ...
      <k-form-item label="资产代码" data-label-width="150px">
        <k-field-select v-model="searchParam.scrId" data-action="GridAffiliateOtherAnalysis.findScrIdAndscrNm" :dataRemote="true"
        ...
        <k-form-item label="交易类型" data-label-width="150px">
            <k-field-select v-model="searchParam.dealType"  data-dict ="bus_typ_cd"/>
          </k-form-item>
    ...
    <div>
      <k-grid ref="GridAffiliateOtherAnalysisGrid" @data-row-select="selectRow" data-fixed="right" data-operate-width="250px"
              data-action="GridAffiliateOtherAnalysis.findGridAffiliateOtherAnalysis">
        ...
        <k-grid-column data-align="left" data-header="交易类型" data-name="dealType"  data-dict ="bus_typ_cd"></k-grid-column>
        ...
        <template slot="operate" slot-scope="scope">
          <k-btn class="btn-custom-plain" data-descript="修改其他交易关联方数据" data-functype="POPUP" data-size="mini"
                 data-target="editGridAffiliateOtherAnalysisPopup" :openType="'edit'" v-if="global.isShowAuthorityButton('GridAffiliateOtherAnalysis.updateGridAffiliateOtherAnalysis')">
            修改
          </k-btn>
           <k-btn class="md-danger" data-functype="SUBMIT" data-action="GridAffiliateOtherAnalysis.deleteGridAffiliateOtherAnalysis" data-size="mini"
              data-type="danger" data-target="GridAffiliateOtherAnalysisGrid" :data-confirm="true" data-descript="删除" v-if="global.isShowAuthorityButton('GridAffiliateOtherAnalysis.deleteGridAffiliateOtherAnalysis')">
              删除
          </k-btn>
        </template>
      </k-grid>

        <k-popup ref="editGridAffiliateOtherAnalysisPopup" data-title="修改">
                  <k-form ref="editGridAffiliateOtherAnalysisForm" :data-col="2" isFormBodyScreen>
                      ...
                      <k-form-item label="交易类型">
                          <k-field-select  v-model="formData.dealType"    data-dict ="bus_typ_cd" :data-allowblank="true" :data-disabled="false" />
                      </k-form-item>
                      ...
                      <k-btn class="btn-custom-primary" data-functype="SUBMIT" data-action="GridAffiliateOtherAnalysis.updateGridAffiliateOtherAnalysis" data-from="editGridAffiliateOtherAnalysisForm"
                          :data-model="formData" data-target="GridAffiliateOtherAnalysisGrid">
...
export default {
  name:"GridAffiliateOtherAnalysis",
  data() {
    return {
      addDocTypeDict:{},
      formData: {},
