<template>
  <div>
    <k-form ref="editNetReportRulesForm" :data-col="2" isFormBodyScreen >
      ...
      <k-form-item label="规律开放周期" v-show="formData.operationMode==='2'">
        <k-field-select v-model="formData.regularOpenCycle" :data-data="regularOpenCycle0" :data-allowblank="formData.operationMode!=='2'"
                        data-display-field="itemkey,itemval" data-value-field="itemkey" :data-disabled="disabledVal"/>
      </k-form-item>
      <k-form-item label="规律开放周期" v-show="formData.operationMode==='3'">
        <k-field-select v-model="formData.regularOpenCycle" :data-data="regularOpenCycle1" :data-allowblank="formData.operationMode!=='2'"
                        data-display-field="itemkey,itemval" data-value-field="itemkey" :data-disabled="true"/>
      </k-form-item>
    ...
    <k-form ref="reportRulesForm1" :data-col="2">
      ...
      <k-form-item label="报送规则" v-show="formData.operationMode!=='1'">
        <k-field-select v-model="formData.reportRules" data-dict="report_rules" :data-allowblank="formData.operationMode==='1'" :data-disabled="formData.operationMode==='3'"
                        data-display-field="itemkey,itemval" data-value-field="itemkey" @data-on-change="reportRulesChange"/>
      </k-form-item>
      <k-form-item label="报送规则" v-show="formData.operationMode==='1'">
        <k-field-select v-model="formData.reportRules" :data-data="reportRulesDict" :data-allowblank="formData.operationMode!=='1'"
                        data-display-field="itemkey,itemval" data-value-field="itemkey" @data-on-change="reportRulesChange"/>
      </k-form-item>
      <k-form-item label="报送估值日" >
        <k-field-select v-model="formData.reportDate" data-dict="report_date" :data-allowblank="false"
                        data-display-field="itemkey,itemval" data-value-field="itemkey" :data-disabled="formData.operationMode==='3'"/>
      </k-form-item>
    ...
    <k-form ref="reportRulesForm2" :data-col="2" v-if="formData.reportRules==='3'">
      ...
      <k-form-item label="月末是否报送" style="margin-top: 20px">
        <k-field-checkbox v-model="formData.reportMonth" data-dict="1yes2no" :data-allowblank="false"
                          data-display-field="itemval" data-value-field="itemkey" :data-multiple="false"/>
      </k-form-item>
      <k-form-item label="确认日是否报送" style="margin-top: 20px">
      ...
      <div :data-col="2">
        ...
        <k-field-select v-model="formData.reportFreq" data-dict="report_freq" @data-on-change="reportFreqChange"
                        class="md-padding-left-10" style="width: 65px; margin-left: 10px" />
        <span style="color: #F56C6C;margin-top: 3px;">*</span>
  ...
  methods: {
    ...
    submitHandler(val){
      ...
      if (val.reportRules === '3'){//固定频率报送
        let fromBool3 = this.$refs.reportRulesForm2.validate();
        if(!fromBool3)
          return false;
    ...
    operationModeChange(val){
      let reportRules = this.formData.reportRules;

      if(reportRules === '2')
        this.$set(this.formData,'reportRules','');

      this.$set(this.formData,'regularOpenCycle','');

      if(val === '3'){
        this.$set(this.formData,'regularOpenCycle','1');
        this.$set(this.formData,'reportRules','1');
        this.$set(this.formData,'reportDate','2');
      }

      this.reportRulesChange(this.formData.reportRules);
    },

    reportRulesChange(val){
      if (val !== '3'){//不是固定频率报送，将数据置空
        this.$set(this.formData,'lengthFreq','');
        this.$set(this.formData,'reportFreq','');
        this.$set(this.formData,'specificDate','');
        this.$set(this.formData,'reportMonth','');
        this.$set(this.formData,'reportConfirmDate','');
      }
      if (val === '3'){//固定频率报送
        this.$set(this.formData,'reportMonth','01');
        this.$set(this.formData,'reportConfirmDate','01');
      }
