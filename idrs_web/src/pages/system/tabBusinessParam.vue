<template>
  <div class="tab-page">
    <k-form-search data-target="BusinessParamGrid" data-model-name="SystemParam" data-label-width="70px" />

    <k-grid ref="BusinessParamGrid" data-action='SystemParam.findBusinessParams' :data-autoload="true" :dataPopupAppendToBody="true"
...
<!--      <k-grid-column data-header="分组ID" data-name="groupparaid"></k-grid-column>-->
      ...
      <template slot="operate">
        <k-btn data-functype="POPUP" data-confirm data-size="mini" class="md-info md-just-icon md-simple"
               data-target="editBusinessParamPopup" data-descript="修改参数" v-if="global.isShowAuthorityButton('SystemParam.updateSysParam')">
    ...
    <!--   修改   -->
    <k-popup ref="editBusinessParamPopup" data-title="修改">
      <k-form ref="editBusinessParamForm" :data-col="2">
        ...
        <k-form-footer data-align="center">
          <k-btn class="btn-custom-primary" data-functype="SUBMIT" data-action="SystemParam.updateSysParam"
                 data-from="editBusinessParamForm" :data-model="formData" :data-handler="changeConfigOptionToStr"
                 data-target="BusinessParamGrid">
...
export default {
  ...
  methods: {
    ...
    updSuccess(pop) {
      this.$refs.BusinessParamGrid.load()
      pop.close()
    }
  }
}
