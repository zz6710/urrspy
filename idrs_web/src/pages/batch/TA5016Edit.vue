  ...
  <md-card class="head-div " id="mainBody">
    ...
    <k-popup ref="addClearGroup" data-title="添加清算组">
      <k-form ref="addForm" :data-col="1">
        <k-form-item label="清算任务类型" v-show="false">
          <k-field-select v-model="editTasktype" data-placeholder="请选择清算组类型"  data-dict="task_type" data-default-value="9" data-disabled/>
        </k-form-item>
...
<script>
import ClearGroupEdit from "@/pages/batch/TA5016-ClearGroupEdit";
...
//对map对象赋值
    ...
    submit() {
      ...
      this.httpUtil.comnUpdate({
        action: 'Ta5015Combin.submit',
        params: {'clearGroupListStr': clearDataStr},
        successAlert: true,
      }).then(data => {
        this.reload = false;
        this.isActive = false;
