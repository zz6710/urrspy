<template>
  <div class="py-page">
    <div>
      <k-form-search-customize data-model-name="SubseqSubscrRegist" data-label-width="130px" data-target="subseqSubscrRegistGrid" v-model = "searchParam">
      <k-form-item label="操作日期">
        ...
        <k-form-item label="业务结束日">
          <k-field-date v-model="BusinessDate" data-type="daterange" data-date-format="yyyyMMdd" data-value-format="yyyyMMdd"/>
        </k-form-item>
      </k-form-search-customize>
    </div>
    <div class="py-page-container">
      <k-grid ref="subseqSubscrRegistGrid" @data-row-select="selectRow" data-operate-column="false" data-action="SubseqSubscrRegist.findSubseqSubscrRegists" >
       ...
       <k-grid-column data-align="left" data-header="操作类型" data-name="opType" data-dict="op_type" data-width="80"></k-grid-column>
        <k-grid-column data-align="left" data-header="产品登记编码" data-name="prodCode" data-width="180"></k-grid-column>
...
<script>
  export default {
    ...
    watch: {
      ...
      BusinessDate() {
        this.$set(this.searchParam, 'busiStartDate', this.BusinessDate == null ? '' : this.BusinessDate[0]);
        this.$set(this.searchParam, 'busiEndDate', this.BusinessDate == null ? '' : this.BusinessDate[1]);
      },
    }
  };
</script>
