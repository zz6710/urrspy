	...
	<div class="py-page">
		<div>
			<k-form-search-customize
				data-model-name="ProdConsignmentSales"
				data-label-width="80px"
				v-model="queryDate"
				data-target="prodConsignmentSalesGrid"
			>
				<k-form-item label="报送日期">
					<k-field-date v-model="searchParam.reportDate" data-type="daterange" data-date-format="yyyyMMdd" data-value-format="yyyyMMdd" />
				</k-form-item>
			</k-form-search-customize>
		</div>
		<div class="py-page-container">
			<div class="table-top-btns">
				<k-btn
					...
					data-target="prodConsignmentSalesGrid"
					data-export-name="委托销售产品编码导入"
					data-url="ProdConsignmentSales.findProdConsignmentSales"
					data-excel-template="委托销售产品编码导入.xlsx"
					data-excel-start-line="3"
					data-template-name="委托销售产品编码导入导出"
				>
			...
			<k-grid
				ref="prodConsignmentSalesGrid"
				data-action="ProdConsignmentSales.findProdConsignmentSales"
				:data-operate-column="false"
				:data-autoload="false"
			>
				<k-grid-column data-header="委托销售产品编码导入" data-align="center">
...
export default {
	...
	mounted() {
		this.getOrgNo();
		this.$nextTick(() => {
			this.$refs.prodConsignmentSalesGrid.load(this.queryDate);
		});
	},
	methods: {
		getOrgNo() {
