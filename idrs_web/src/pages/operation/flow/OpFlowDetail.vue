      ...
      <el-timeline-item
        ...
        <div style="font-size: 16px;margin-top:5px;">
          ...
          <template v-else-if="logData.operationType==8">
            <span :style="{color: operationTypeDict[logData.operationType].color}">待操作</span>
          </template>
          <template v-else>
        ...
        <div v-if="logData.operationType<'4'">
          <template v-for="item in logData.forms">
            ...
            <template v-if="item.formType=='0'">
              <!-- 参数表单，用k-form展示 -->
              <k-form :ref="item.formId" :key="item.formId">
                <template v-for="(param, paramIndex) in item.params">
                  <k-form-item :label="param.paramName" :key="param.paramCode">
    ...
    if (processInstanceId) {
      this.httpUtil.comnQuery({
        ...
        this.logDatas.forEach(logData => {
          ...
          console.log('logdata.forms ---111',logData)
