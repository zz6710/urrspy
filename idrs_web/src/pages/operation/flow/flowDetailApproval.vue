                ...
                <template v-for="(param, paramIndex) in item.params">
                  <k-form-item :label="param.paramName" :key="param.paramCode" :data-col="2">
                    <div style="width:100%;border: 1px solid #DCDFE6;background-color: #f7f7f7;border-radius: 4px;line-height: 32px;font-size: 13px;display: inline-block;padding: 0 15px;"
                      ...
                      <span>{{logData.formData[item.formId][param.paramCode]}}</span>
                    </div>
                    <component v-else v-model="logData.formData[item.formId][param.paramCode]" v-bind="inputOptions(param)" :is="funcTypeMap[param.funcType]"></component>
                  </k-form-item>
                </template>
              </k-form>
            </template>
