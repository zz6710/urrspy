<template>
  <div class="my-process-designer">
    <div class="my-process-designer__header">
        ...
        <el-button-group key="file-control">
          ...
          <el-tooltip effect="light">
            <div slot="content">
              ...
              <br />
              <el-button :size="headerButtonSize" type="text" @click="downloadProcessAsJson()">下载为JSON文件(包含流程参数与表单配置)</el-button>
            </div>
      ...
      <!-- 用于打开本地文件-->
      <input type="file" id="files" ref="refFile" style="display: none" accept=".xml, .bpmn,.json" @change="importLocalFile" />
    </div>
    <div class="my-process-designer__container">
      <div class="my-process-designer__canvas" ref="bpmn-canvas"></div>
    </div>
    <k-popup ref="previewPopup" data-title="预览" data-width="60%" >
      <highlightjs :language="previewType" :code="previewResult" style="height: 60vh" />
    </k-popup>
    <!-- <el-dialog title="预览" width="60%" :visible.sync="previewModelVisible" destroy-on-close append-to-body>
    </el-dialog> -->
  </div>
</template>
...
import { getEnvId, getEnvItems } from "../utils";
...
export default {
	...
	methods: {
		...
		async downloadProcess(type) {
			//获取流程名
			let name = window.bpmnInstances.modeler.getDefinitions().rootElements.filter((t) => t.$type === "bpmn:Process")[0]?.name;
			try {
				const _this = this;
				// 按需要类型创建文件并下载
				if (type === "xml" || type === "bpmn") {
					...
					const { err, xml } = await this.bpmnModeler.saveXML();
					let currentXml = cloneDeep(xml);
					// 读取异常时抛出异常
					if (err) {
						console.error(`[Process Designer Warn ]: ${err.message || err}`);
					}
					let processJson = convert.xml2json(xml, { spaces: 2 });
					//下载对应的流程参数与表单配置
					Promise.all([this.getEnvItemList(), this.getEnv(), this.getFormFields(currentXml)]).then((data) => {
						let config = {
							process: JSON.parse(processJson),
							envItems: this.envItems,
							env: this.env,
							formFields: this.formFields,
						};
						console.log("config", config);
						let { href, filename } = _this.setEncoded("json".toUpperCase(), name, JSON.stringify(config));
						downloadFunc(href, filename);
					});
				} else {
					...
					let { href, filename } = _this.setEncoded("SVG".toUpperCase(), name, svg);
					downloadFunc(href, filename);
				}
			} catch (e) {
				console.error(`[Process Designer Warn ]: ${e.message || e}`);
		...
		getEnv() {
			...
			if (this.envId) {
				return this.httpUtil
					.ajaxJson({
						url: "wf/env/get/" + this.envId + ".json",
					})
					.then((res) => {
						this.env = res.data;
					});
		...
		getFormFields(xml) {
			...
			return this.httpUtil
				.ajaxJson({
					url: "wf/formField/listByXml.json",
					params: { bpmnXml: xml },
				})
				.then((res) => {
		...
		setEncoded(type, filename = "diagram", data) {
			...
			if (type === "svg") {
				...
				applicationType = "bpmn20-xml";
			}
			return {
				filename: `${filename}.${type}`,
				href: `data:application/${applicationType};charset=UTF-8,${encodedData}`,
				data: data,
			};
		},
		...
		importLocalFile() {
			...
			reader.onload = function () {
				...
				if (file.type.indexOf("json") > -1) {
					//处理json文件
					let config = JSON.parse(xmlStr);
					//处理流程
					xmlStr = convert.json2xml(JSON.stringify(config.process));
					//处理流程参数与表单
					that.importConfig(config);
				}
		...
		importConfig(config) {
			return this.httpUtil
				.ajaxJson({
					url: "wf/model/importConfig.json",
					params: config,
				})
				.then((res) => {
					this.formFields = res.rows;
					Tools.alert("已成功导入流程参数配置与表单配置");
				});
		},
		/* ------------------------------------------------ refs methods ------------------------------------------------------ */
		...
		downloadProcessAsJson() {
			this.downloadProcess("json");
		},
		processSimulation() {
