<template>
  <div class="panel-tab__content">
    <el-table :data="elementPropertyList" size="mini" max-height="240" border fit>
      <el-table-column label="属性名" prop="name" min-width="95px" show-overflow-tooltip />
      <el-table-column label="属性值" prop="value" min-width="95px" show-overflow-tooltip />
    ...
    <el-drawer :visible.sync="propertyFormModelVisible" title="属性配置" :size="`${width}px`" destroy-on-close>
...
export default {
	...
	methods: {
		resetAttributesList() {
			...
			this.bpmnElementPropertyList = this.bpmnElementPropertyList.filter((t) => t !== undefined);
		},
		copyAttributesList() {
			// 复制 显示
