<template>
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="mini" border>
      <el-table-column label="事件类型" min-width="95px" show-overflow-tooltip :formatter="row => listenerEventTypeObject[row.event]" />
      <el-table-column label="监听器类型" min-width="95px" show-overflow-tooltip :formatter="row => listenerTypeObject[row.listenerType]" />
    ...
    <!-- 监听器 编辑/创建 部分 -->
    <el-drawer :visible.sync="listenerFormModelVisible" title="任务监听器" :size="`${width}px`" destroy-on-close>
      <div class="element-drawer__content">
        <el-form size="mini" :model="listenerForm" label-width="96px" ref="listenerFormRef" @submit.native.prevent>
          ...
          <el-form-item v-if="listenerForm.listenerType === 'paramListener' || listenerForm.listenerType === 'paramListener'" label="流程参数" prop="expression" key="listener-param" :rules="{ required: true, trigger: ['blur', 'change'] }">
            <el-select v-model="listenerForm.expression" size="mini" clearable placeholder="请选择 流程参数">
          ...
          <template v-if="listenerForm.listenerType === 'scriptListener'">
            <el-form-item label="脚本格式" prop="scriptFormat" key="listener-script-format" :rules="{ required: true, trigger: ['blur', 'change'], message: '请填写脚本格式' }">
              <el-select v-model="listenerForm.scriptFormat" clearable>
                <el-option v-for="i in Object.keys(scriptTypeObject)" :key="i" :label="scriptTypeObject[i]" :value="i" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item v-if="listenerForm.scriptType === 'inlineScript'" label="脚本内容" prop="value" key="listener-script" :rules="{ required: true, trigger: ['blur', 'change'], message: '请填写脚本内容' }">
              <el-input v-model="listenerForm.value" clearable type="textarea" />
            </el-form-item>
    ...
    <!-- 注入西段 编辑/创建 部分 -->
    <el-drawer title="字段配置" :visible.sync="listenerFieldFormModelVisible" :size="`${width}px`" destroy-on-close>
      <div class="element-drawer__content">
...
import { initListenerForm, initListenerType, eventType, listenerType, fieldType, scriptType } from "./utilSelf";
	...
	methods: {
		getEnvItemList() {
			getEnvItems().then((rows) => {
				this.envItems = rows
					...
					.map((t) => {
						//拼接表达式
						t.itemValue = "${parseWfParamListener.parseTaskParam('" + t.envItemId + "',task)}";
						return t;
					});
			});
		},
		...
		openListenerForm(listener, index) {
			if (listener) {
				...
				this.listenerForm = {};
				this.$set(this.listenerForm, "scriptType", "inlineScript");
