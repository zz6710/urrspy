...
export function createFieldObject(option, prefix) {
  ...
  const fieldConfig = string ? { name, string } : { name, expression };
...
// 获取流程参数id
export async function getEnvItems() {
  let envId = getEnvId();
  let envItems = []
  if (envId) {
    await httpUtil
...
export function getEnvId() {
  return window.bpmnInstances.modeler.getDefinitions().rootElements
    .filter((t) => t.$type === "bpmn:Process")[0]
    .extensionElements?.values?.filter((t) => t.$type == `flowable:Env`)[0]?.id;
}
